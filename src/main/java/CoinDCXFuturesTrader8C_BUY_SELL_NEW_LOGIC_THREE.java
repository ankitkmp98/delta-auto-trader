...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // =========================================================================
    // NEW — Quick-profit monitor. This is INDEPENDENT of the fixed SL/TP
    // below — it does not use any candle timeframe or EMA, since the goal
    // ("close as soon as ₹10 profit is reached") is a direct live-price
    // check, not a trend/indicator decision. A candle-based check would
    // only refresh once per candle-close and could miss a brief profit
    // window; polling the live price every few seconds catches it directly.
    //
    // NOTE: this can trigger BEFORE the fixed TP is reached (₹10 profit on
    // a ~₹1500 position is a much smaller move than the RR_TARGET-based
    // TP), so most winning trades will likely close via this path, not via
    // the fixed TP. The fixed SL is untouched — a losing trade still only
    ...
    // =========================================================================
    // Direction -> 1H+30M, Setup -> 15M, Entry -> 5M. UNCHANGED.
    // =========================================================================
    ...
    // UNCHANGED — this is the logic you asked to keep exactly as-is. ----
    ...
    // =========================================================================
    // FIXED SL/TP — UNCHANGED, exactly as it was.
    // =========================================================================
    ...
    // =========================================================================
    // Orchestrator — quick-profit monitor now runs every cycle (5s), entry
    // scan still runs every 20s on its own timer.
    // =========================================================================
    public static void main(String[] args) {
        System.out.println("=== Scalp bot starting (fixed SL/TP + quick-profit monitor) ===");
        initInstrumentCache();
        ...
        while (true) {
            ...
            try {
                TimeUnit.MILLISECONDS.sleep(QUICK_PROFIT_SCAN_INTERVAL_MS);
    ...
    // =========================================================================
    // NEW — Quick-profit monitor. Pure live-price check, no candles/EMA.
    // Computes gross PnL (in INR) for every open position; closes the FULL
    // position the moment it reaches QUICK_PROFIT_TARGET_INR.
    // =========================================================================
    private static void monitorQuickProfitPositions() {
        ...
        for (String pair : active) {
            try {
                ...
                if (avgPrice <= 0 || posQty == 0) continue;

                boolean isLong = posQty >= 0;
                double qty = Math.abs(posQty);

                double currentPrice = getLastPrice(pair);
                if (currentPrice <= 0) continue;

                double pnlUsdt = isLong
                        ? (currentPrice - avgPrice) * qty
                        : (avgPrice - currentPrice) * qty;

                double usdtInrRate = 98.0;
                double pnlInr = pnlUsdt * usdtInrRate;

                if (pnlInr >= QUICK_PROFIT_TARGET_INR) {
                    System.out.printf("[QUICK-PROFIT] %s | unrealized PnL=₹%.2f >= target ₹%.2f — closing position%n",
                            pair, pnlInr, QUICK_PROFIT_TARGET_INR);
                    exitPosition(pos.optString("id", null), pair);
                }
            } catch (Exception e) {
                System.err.println("monitorQuickProfitPositions(" + pair + "): " + e.getMessage());
            }
        }
    }
    ...
    private static void exitPosition(String posId, String pair) {
        if (posId == null) {
            System.out.println("  [QUICK-PROFIT] " + pair + " — position ID missing, cannot exit");
            return;
        }
        try {
            ...
            System.out.println("  [QUICK-PROFIT] " + pair + " exit response: " + resp);
        } catch (Exception e) {
