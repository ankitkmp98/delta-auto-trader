...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // =========================================================================
    // NEW — Breakeven-lock (re-added, SAFE version only). Protects against
    // sudden flash-dips/stop-hunting wicks that occur AFTER a trade has
    // already moved meaningfully favorable but BEFORE the fixed TP is hit —
    // exactly the scenario you described. Once a position has covered
    // BREAKEVEN_LOCK_TRIGGER_FRACTION of its original TP-distance, SL is
    // moved ONCE to a small guaranteed-profit level above entry. The fixed
    // TP is NEVER touched by this — this avoids the earlier bug where
    // continuously recomputing TP made it unreachable. This is standard
    // professional practice: lock in downside protection once a trade is
    // meaningfully in profit, rather than leaving full risk exposed all
    // the way to the target.
    // =========================================================================
    private static final double BREAKEVEN_LOCK_TRIGGER_FRACTION = 0.5;   // activate at 50% of TP-distance
    private static final double BREAKEVEN_LOCK_PROFIT_PERCENT   = 0.30;  // lock entry + 0.30% (not just breakeven)
    private static final long   BREAKEVEN_POLL_INTERVAL_MS      = 5_000L; // check every 5s — fast enough to
    // lock in before a brief flash-dip can wipe out the favorable move.

    private static class BreakevenState {
        boolean isLong;
        double entryPrice;
        double initialReward; // TP-distance from entry, captured once at order time
        boolean locked;
    }

    private static final Map<String, BreakevenState> breakevenStateMap = new ConcurrentHashMap<>();
    ...
    public static void main(String[] args) {
        System.out.println("=== Scalp bot starting (fixed margin, fixed SL/TP + breakeven-lock) ===");
        initInstrumentCache();
        reconcileBreakevenStateOnStartup();
        ...
        while (true) {
            ...
            try {
                TimeUnit.MILLISECONDS.sleep(BREAKEVEN_POLL_INTERVAL_MS);
    ...
    // Reconstructs breakeven-tracking for any positions that were already
    // open when the bot (re)started, using their current SL/TP as the
    // reference point (best-effort — treats them as not-yet-locked).
    private static void reconcileBreakevenStateOnStartup() {
        try {
            ...
            for (String pair : active) {
                ...
                if (avgPrice <= 0 || tpTrig <= 0) continue;

                BreakevenState state = new BreakevenState();
                state.isLong = posQty >= 0;
                state.entryPrice = avgPrice;
                state.initialReward = Math.abs(tpTrig - avgPrice);
                state.locked = false;
                breakevenStateMap.put(pair, state);
            }
        } catch (Exception e) {
            System.err.println("reconcileBreakevenStateOnStartup: " + e.getMessage());
        }
    }

    // =========================================================================
    // NEW — Breakeven-lock check. Runs every BREAKEVEN_POLL_INTERVAL_MS
    // (5s), independent of the entry-scan cadence, so it can react quickly
    // to a favorable move before a flash-dip can undo it.
    // =========================================================================
    private static void breakevenLockOpenPositions() {
        try {
            Set<String> active = getActivePositions();
            breakevenStateMap.keySet().removeIf(pair -> !active.contains(pair));

            for (String pair : active) {
                BreakevenState state = breakevenStateMap.get(pair);
                if (state == null || state.locked || state.initialReward <= 0) continue;
                ...
                double newSL = state.isLong
                        ? state.entryPrice * (1 + BREAKEVEN_LOCK_PROFIT_PERCENT / 100.0)
                        : state.entryPrice * (1 - BREAKEVEN_LOCK_PROFIT_PERCENT / 100.0);
                newSL = roundToTick(newSL, tick);

                boolean alreadyBeyond = state.isLong ? curSL >= newSL : curSL <= newSL;
                if (alreadyBeyond) {
                    state.locked = true;
                    continue;
                }
                ...
                // TP (curTP) is passed through UNCHANGED — only SL moves.
                System.out.printf("[BE-LOCK] %s | price=%.6f | SL %.6f -> %.6f (TP unchanged at %.6f)%n",
                        pair, currentPrice, curSL, newSL, curTP);
                setTpSl(posId, curTP, newSL, pair);

                try {
                    TimeUnit.MILLISECONDS.sleep(800);
                    JSONObject verify = findPosition(pair);
                    if (verify != null && verify.optDouble("stop_loss_trigger", 0) > 0) {
                        state.locked = true;
                    }
        ...
        try {
            ...
            for (String pair : stillOpen) {
                ...
                if (posId != null) {
                    ...
                    if (confirmed && !breakevenStateMap.containsKey(pair)) {
                        BreakevenState state = new BreakevenState();
                        state.isLong = isLong;
                        state.entryPrice = avgPrice;
