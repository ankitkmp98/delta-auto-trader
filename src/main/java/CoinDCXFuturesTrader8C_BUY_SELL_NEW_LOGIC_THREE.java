...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // ── FIX (loss-reduction round 1) ─────────────────────────────────────────
    // Problem 1: SL was getting hit almost immediately after entry.
    //   Cause: SL was purely "1H ST line ± buffer" — in a strong trend the ST
    //   line sits very close to price, producing an unrealistically tight
    //   stop that normal 15m noise blows through.
    //   Fix: enforce a MINIMUM stop distance in 15m-ATR terms, on top of the
    //   ST-line-based stop. Whichever is WIDER wins.
    private static final double SL_MIN_ATR_15M = 1.4;

    // Problem 1b: entering right on the close of the rejection candle can
    // catch a fake bounce (price immediately resumes the pullback and stops
    // you out). Fix: require the NEXT 15m candle to actually break past the
    // rejection candle's extreme in the trade direction before entering —
    // i.e. wait for confirmation instead of assuming the rejection holds.
    private static final boolean REQUIRE_BREAKOUT_CONFIRMATION = true;

    // Problem 2: trades taken inside choppy/range-bound conditions get
    // stopped both ways. Fix: bring back a lightweight ADX(1H) trend-strength
    // gate (not the whole old multi-gate system — just this one check).
    private static final int    ADX_PERIOD = 14;
    private static final double ADX_MIN    = 18.0;

    // Problem 3: TP never gets reached, price reverses and gives back the
    // move. Fix: once price has moved 1R in favor, move SL to breakeven
    // (locks in "can't lose" instead of hoping for full TP), AND exit fully
    // if the 15m Supertrend flips against the position or EMA9/21 crosses
    // against it (matches the document's own trailing-exit rule, which the
    // first rewrite had dropped).
    private static final double BREAKEVEN_TRIGGER_R = 1.0;
    ...
    // =========================================================================
    private static double[] computeSlTp(boolean isLong, double entryPrice, TFResult tf1h, double atr15, double tickSize) {
        double sl, tp;
        if (isLong) {
            ...
            // FIX: enforce a minimum stop distance (in 15m ATR) so the stop
            // isn't unrealistically tight when the 1H ST line sits close to price.
            double minDistFloor = entryPrice - SL_MIN_ATR_15M * atr15;
            raw = Math.min(raw, minDistFloor); // "wider" for a long = lower price
            ...
            double minDistCeil = entryPrice + SL_MIN_ATR_15M * atr15;
            raw = Math.max(raw, minDistCeil); // "wider" for a short = higher price
    ...
    public static void main(String[] args) {
        ...
        // FIX: manage already-open positions FIRST — move SL to breakeven
        // once 1R profit is banked, and exit fully if the 15m trend flips
        // against the position, instead of only hoping fixed TP gets hit.
        manageOpenPositions();
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                // ─────────────────────────────────────────────────────────────
                // FIX: ADX(1H) trend-strength gate — avoids taking trades
                // inside choppy/range-bound conditions where SL gets hit both
                // ways. This was fully removed in the first rewrite; bringing
                // back just this one check (not the whole old gate stack).
                // ─────────────────────────────────────────────────────────────
                double adx1h = calcADX(tf1h.hi, tf1h.lo, tf1h.cl, ADX_PERIOD);
                System.out.printf("  [ADX-1H] ADX=%.2f (min=%.0f) → %s%n",
                        adx1h, ADX_MIN, adx1h >= ADX_MIN ? "PASS" : "FAIL — choppy");
                if (adx1h < ADX_MIN) {
                    System.out.println("  ADX FAIL — market not trending enough, avoid whipsaw — skip"); continue;
                }
                ...
                // Rejection candle = second-most-recent CLOSED 15m candle.
                // Confirmation candle = the most recent CLOSED 15m candle —
                // it must actually break past the rejection candle's extreme
                // in the trade direction (FIX: avoids catching a fake bounce
                // that immediately resumes the pullback and stops you out).
                if (n15 < 4) { System.out.println("  Not enough 15m candles for entry check — skip"); continue; }
                double rejClose = cl15[n15 - 3], rejOpen = op15[n15 - 3];
                double rejHigh  = hi15[n15 - 3], rejLow  = lo15[n15 - 3];
                double prevClose = cl15[n15 - 4], prevOpen = op15[n15 - 4];

                // Pullback check on the rejection candle: close to 9EMA / 21EMA / ST line
                double distEma9  = Math.abs(rejClose - ema9_15);
                double distEma21 = Math.abs(rejClose - ema21_15);
                double distSt    = trendUp
                        ? Math.abs(rejClose - stBands15[0])
                        : Math.abs(rejClose - stBands15[1]);
                ...
                boolean rejectionOk = trendUp
                        ? isBullishRejection(rejOpen, rejHigh, rejLow, rejClose, prevOpen, prevClose)
                        : isBearishRejection(rejOpen, rejHigh, rejLow, rejClose, prevOpen, prevClose);
                ...
                if (REQUIRE_BREAKOUT_CONFIRMATION) {
                    double confClose = cl15[n15 - 2], confHigh = hi15[n15 - 2], confLow = lo15[n15 - 2];
                    boolean brokeOut = trendUp
                            ? (confClose > rejHigh)   // confirmation candle closed above the rejection candle's high
                            : (confClose < rejLow);   // confirmation candle closed below the rejection candle's low
                    System.out.printf("  [15M-Confirm] confClose=%.6f vs rejExtreme=%.6f → %s%n",
                            confClose, trendUp ? rejHigh : rejLow, brokeOut ? "BROKE OUT — confirmed" : "not confirmed yet");
                    if (!brokeOut) {
                        System.out.println("  15M FAIL — rejection candle not yet confirmed by breakout — skip"); continue;
                    }
                }
                System.out.println("  15M OK — pullback + rejection candle + breakout confirmed");
                ...
                double[] slTp = computeSlTp(trendUp, entry, tf1h, atr15, tickSize);
    ...
    // =========================================================================
    // FIX (loss-reduction round 1): manage already-open positions every scan.
    //   a) Trend-flip exit — if the 15m Supertrend flips against the position
    //      OR EMA9/21 crosses against it, close the position at market
    //      immediately instead of waiting for a possibly-distant fixed TP
    //      (this is the document's own "trailing exit" rule).
    //   b) Breakeven move — once price has moved BREAKEVEN_TRIGGER_R (1.0R)
    //      in favor, move the SL to entry price so the trade can no longer
    //      lose money even if it later reverses before hitting full TP.
    // =========================================================================
    private static void manageOpenPositions() {
        try {
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                if (avgPrice <= 0 || posQty == 0 || posId == null || pair.isEmpty()) continue;
                boolean isLong = posQty > 0;

                JSONArray raw15m = getCandlestickData(pair, "15", CANDLE_15M);
                if (raw15m == null || raw15m.length() < EMA_MID + 5) continue;
                ...
                boolean[] stSeries = calcSupertrend(hi15, lo15, cl15, ST_PERIOD, ST_MULTIPLIER);
                boolean stBull = stSeries[stSeries.length - 1];

                boolean trendFlippedAgainst = isLong
                        ? (!stBull || ema9 < ema21)
                        : (stBull  || ema9 > ema21);

                if (trendFlippedAgainst) {
                    System.out.println("  [MANAGE] " + pair + " — 15m trend flipped against position, exiting at market");
                    exitPosition(pair, posId);
                    continue; // position closed, nothing else to manage for it
                }

                double slTrig = pos.optDouble("stop_loss_trigger", 0);
                double tpTrig = pos.optDouble("take_profit_trigger", 0);
                if (slTrig <= 0) continue; // no SL to move yet — safety sweep will handle it

                double risk = Math.abs(avgPrice - slTrig);
                if (risk <= 0) continue;
                boolean alreadyBreakeven = isLong ? slTrig >= avgPrice : slTrig <= avgPrice;
                if (alreadyBreakeven) continue;

                double currentPrice = getLastPrice(pair);
                if (currentPrice <= 0) continue;
                double favorableMove = isLong ? (currentPrice - avgPrice) : (avgPrice - currentPrice);

                if (favorableMove >= BREAKEVEN_TRIGGER_R * risk) {
                    double tick  = getTickSize(pair);
                    double newSl = roundToTick(avgPrice, tick);
                    double tp    = tpTrig > 0 ? tpTrig
                            : roundToTick(isLong ? avgPrice + RR_TARGET * risk : avgPrice - RR_TARGET * risk, tick);
                    System.out.printf("  [MANAGE] %s reached %.1fR — moving SL to breakeven (%.6f)%n",
                            pair, BREAKEVEN_TRIGGER_R, newSl);
                    setTpSlWithRetry(posId, tp, newSl, pair);
                }
            }
        } catch (Exception e) {
            System.err.println("manageOpenPositions: " + e.getMessage());
        }
    }

    // CoinDCX confirmed endpoint: POST /exchange/v1/derivatives/futures/positions/exit
    // body: {timestamp, id: position.id} — closes the entire position via market order.
    private static void exitPosition(String pair, String posId) {
        try {
            JSONObject body = new JSONObject();
            body.put("timestamp", Instant.now().toEpochMilli());
            body.put("id", posId);
            String resp = authPost(BASE_URL + "/exchange/v1/derivatives/futures/positions/exit", body.toString());
            System.out.println("  [MANAGE] exit response for " + pair + ": " + resp);
        } catch (Exception e) {
    ...
    private static void ensureTpSlForOpenPositions() {
        try {
            ...
            for (String pair : stillOpen) {
                ...
                double atr15Sweep = (raw15mSweep != null && raw15mSweep.length() >= ATR_PERIOD + 1)
                        ? calcATR(extractHighs(raw15mSweep), extractLows(raw15mSweep), extractCloses(raw15mSweep), ATR_PERIOD)
                        : tf1h.atr; // fallback if 15m fetch fails
                ...
                double tick = getTickSize(pair);
                double[] slTp = computeSlTp(isLong, avgPrice, tf1h, atr15Sweep, tick);
