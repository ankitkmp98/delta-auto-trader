 ...
 *
 *   SL/TP:
 *     Long  SL = clamp(swingLow - 0.5xATR,  entry-1.0xATR, entry-0.5xATR)
 *     Short SL = clamp(swingHigh + 0.5xATR, entry+0.5xATR, entry+1.0xATR)
 *     TP       = entry +/- 0.5 x actual risk  (1:0.5 R:R)
 */
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // SL/TP parameters
    private static final double SL_SWING_BUFFER = 0.5;   // Reduced buffer for stop-loss
    private static final double SL_MIN_ATR      = 0.5;   // Minimum SL distance (5m ATR units)
    private static final double SL_MAX_ATR      = 1.0;   // Maximum SL distance (5m ATR units)
    private static final double RR              = 0.5;   // Adjusting R:R for smaller TP
    ...
    private static final String[] COIN_SYMBOLS = {
        // (Add your coin symbols here)
        "SOL", "ETH", "BTC", "ADA", "XRP", "LTC" // Example coins
    };
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                if (raw15m == null || raw15m.length() < 60) {
                    System.out.println("  Insufficient 15m candles (" + (raw15m == null ? 0 : raw15m.length()) + ") — skip");
                    continue;
                }
                if (raw1h == null || raw1h.length() < EMA_MACRO) {
                    System.out.println("  Insufficient 1H candles (" + (raw1h == null ? 0 : raw1h.length()) + ") — skip");
                    continue;
                }
                if (raw5m == null || raw5m.length() < 30) {
                    System.out.println("  Insufficient 5m candles (" + (raw5m == null ? 0 : raw5m.length()) + ") — skip");
                    continue;
                }
                ...
                JSONObject resp = placeFuturesMarketOrder(side, pair, qty, LEVERAGE, "email_notification", "isolated", "INR");
                ...
                if ("buy".equalsIgnoreCase(side)) {
                    ...
                    tpPrice = entry + 0.5 * risk;  // Smaller TP for more frequent hits
                } else {
    ...
    private static boolean[] calcSupertrend(double[] hi, double[] lo, double[] cl, int period, double multiplier) {
        int n = cl.length;
    ...
    // Rest of your utility functions (calcATR, calcEMA, etc.) would remain unchanged
    ...
    // Implement other methods like getCandlestickData, initInstrumentCache, etc. as in the original code
    ...
    public static JSONObject placeFuturesMarketOrder(String side, String pair, double qty, int lev, String notif, String marginType, String marginCcy) {
        // (Implementation remains unchanged)
    }
