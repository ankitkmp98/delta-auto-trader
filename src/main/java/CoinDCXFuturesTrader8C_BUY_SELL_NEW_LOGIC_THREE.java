...
// }
                ...
                int leverage = 11; // Default leverage
...
private static String determinePositionSide(String pair) {
    try {
        ...
        // FIXED: Always trade - no more random fallback
        ...
        System.out.println("Trend for " + pair + ": " + (priceChange * 100) + "%");
    ...
    try {
        ...
        try {
            ...
            if (freshCandles != null && freshCandles.length() >= 2) {
                double c1 = freshCandles.getJSONObject(freshCandles.length() - 2).getDouble("close");
                double c2 = freshCandles.getJSONObject(freshCandles.length() - 1).getDouble("close");
                return (c2 > c1) ? "buy" : "sell";
            }
        } catch (Exception ex) {
