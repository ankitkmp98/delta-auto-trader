...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // Risk/Capital config
    private static final double MAX_MARGIN = 2100.0;                // Max notional capital used
    private static final double MAX_LEVERAGE_CAP = 10.0;            // Safety cap; do not use full exchange max
    ...
    // Strategy parameters
    private static final int LOOKBACK_PERIOD = 5;                   // Minutes for trend analysis
    private static final double TREND_THRESHOLD = 0.027;            // 2.7% change threshold for trend
    private static final double TP_PERCENTAGE = 0.01;               // 1% take profit (pro-style tighter RR)
    private static final double SL_PERCENTAGE = 0.005;              // 0.5% stop loss
    ...
    private static final String[] COIN_SYMBOLS = {
            ...
            "KSM", "KDA", "IOST", "BAT", "CETUS", "DF", "LRC", "HIVE", "DEGEN", "MTL",
            "SAFE", "CELR", "AVA", "CKB", "RIF", "FIO", "1000000MOG", "KNC", "ICX",
    ...
    public static void main(String[] args) {
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                if (side == null) {
                    System.out.println("⏸ No clear edge for " + pair + " - Skipping trade");
                    continue;
                }

                // Dynamic leverage per pair based on instrument details
                int leverage = getDynamicLeverage(pair);
                System.out.println("Using leverage " + leverage + " for " + pair);
    ...
    // Decide LONG/SHORT using trend + RSI
            ...
            if (currentTime - lastInstrumentUpdateTime > TICK_SIZE_CACHE_TTL_MS) {
                ...
                for (int i = 0; i < activeInstruments.length(); i++) {
                    ...
                    try {
                        ...
                        JSONObject instrumentDetails =
                                new JSONObject(instrumentResponse).getJSONObject("instrument");
                ...
                System.out.println("✅ Successfully updated instrument details for "
                        + instrumentDetailsCache.size() + " pairs");
            }
        } catch (Exception e) {
    ...
    private static int getDynamicLeverage(String pair) {
        JSONObject instrumentDetails = instrumentDetailsCache.get(pair);
        if (instrumentDetails == null) {
            // Fallback if no instrument details; conservative leverage
            return 3;
        }
        double maxLev = instrumentDetails.optDouble("max_leverage", 3.0);
        // Use safe fraction of max leverage
        double usedLev = Math.min(maxLev, MAX_LEVERAGE_CAP);
        // For risk management, optionally use half of allowed:
        usedLev = Math.max(2.0, usedLev * 0.6);
        return (int) Math.floor(usedLev);
    }
    ...
    private static double calculateQuantity(double currentPrice, int leverage, String pair) {
        // Total notional = MAX_MARGIN * leverage / price, but cap quantity sanity
        double notional = MAX_MARGIN * leverage;
        double quantity = notional / currentPrice;

        return Math.max(
                INTEGER_QUANTITY_PAIRS.contains(pair)
                        ? Math.floor(quantity)            // Integer contracts
                        : Math.floor(quantity * 100) / 100, // 2 decimal places
                0
        );
    }
