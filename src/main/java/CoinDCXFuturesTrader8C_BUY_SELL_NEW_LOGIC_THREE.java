...
/**
 * CoinDCX Futures Trader v10 — Full Fix Edition
 *
 * ALL 10 FIXES APPLIED from loss analysis:
 *
 *   FIX-1  RR restored to 2.0 (was 1.0 — guaranteed loss)
 *   FIX-2  Daily P&L limits now ENFORCED in main loop (were defined but never called)
 *   FIX-3  MAX_OPEN_POSITIONS = 3 cap enforced (was unlimited — wiped on reversals)
 *   FIX-4  Live USDT/INR rate fetched (was hardcoded 98 — wrong, causes size error)
 *   FIX-5  RSI zones non-overlapping: Long 52-68, Short 32-48 (was 45-65/35-55 — overlapped)
 *   FIX-6  ATR% volatility filter: skip if < 0.15% (ranging) or > 3.5% (explosive)
 *   FIX-7  Volume confirmation added as S3 — need 2 of 3 soft filters (was 1 of 2)
 *   FIX-8  Minimum R:R validator BEFORE placing order (skip if actual R:R < 1.8)
 *   FIX-9  Meme/low-liquidity coins removed (wide spreads eat R:R before trade starts)
 *   FIX-10 Trade logging to trades.log (measure real win rate over 30+ trades)
 *
 * API CORRECTIONS from official CoinDCX docs:
 *   - candlestick resolution: only '1','5','60','1D' are valid — removed '15' (invalid)
 *     15m is approximated using 5m candles (3x5m = 15m equivalent)
 *     OR use 60m candles for macro + 5m for entry (2-layer instead of 3-layer)
 *   - TP/SL: only "take_profit_market" and "stop_market" are supported order_types
 *   - limit_price in create_tpsl is NOT supported — removed from payload
 *   - margin_currency_short_name default is USDT — INR must be explicit
 *   - position id is FIXED per pair — same id used across sessions
 *   - active_pos is NEGATIVE for short positions
 *   - candlestick response field is "volume" (confirmed from docs)
 *
 * SIGNAL ARCHITECTURE:
 *   4-HARD (all must pass):
 *     H1. 1H EMA-50 macro trend
 *     H2. 1H EMA-9 vs EMA-21 local (using 60m candles — valid resolution)
 *     H3. MACD on 1H
 *     H4. Supertrend(10,3) on 1H
 *
 *   3-SOFT (at least 2 of 3 must pass):
 *     S1. RSI zone (52-68 long, 32-48 short — no overlap)
 *     S2. Previous candle body direction
 *     S3. Volume spike (last candle > 1.3x 10-bar average)
 *
 *   ENTRY TIMING:
 *     Last closed 5m candle must match signal direction
 *     Price within 0.8x ATR of EMA-9
 *
 *   SL/TP:
 *     SL = swing structure +/- noise buffer, clamped 2.0-3.5x ATR
 *     TP = entry +/- 2.0x actual risk (validated >= 1.8 R:R before entry)
 */
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    private static final long COOLDOWN_MS            = 2 * 60 * 60 * 1000L; // 2h per pair
    ...
    // [FIX-5] Non-overlapping RSI zones
    ...
    // SL/TP — [FIX-1] RR restored to 2.0
    private static final double SL_SWING_BUFFER = 0.3;
    private static final double SL_MIN_ATR      = 2.0;
    private static final double SL_MAX_ATR      = 3.5;   // [FIX-1] widened from 2.5
    private static final double RR              = 2.0;   // [FIX-1] restored from 1.0
    private static final double NOISE_BUFFER    = 0.8;
    private static final double MIN_RR_REQUIRED = 1.8;   // [FIX-8]

    // [FIX-6] ATR volatility filter
    private static final double ATR_PCT_MIN = 0.15;
    private static final double ATR_PCT_MAX = 3.5;

    // [FIX-7] Volume spike threshold
    private static final double VOLUME_SPIKE_MULT = 1.3;

    // Candle counts — NOTE: CoinDCX only supports '1','5','60','1D' resolutions
    // We use 60m (1H) for all signal layers + 5m only for entry timing
    private static final int CANDLE_1H  = 150;  // 1H candles for H1-H4 + soft filters
    private static final int CANDLE_5M  = 60;   // 5m candles for entry timing only

    // =========================================================================
    // [FIX-2] Daily P&L Protection — state fields (now actually used)
    // =========================================================================
    ...
    // =========================================================================
    // [FIX-9] Tiered leverage — higher for top liquid pairs
    // =========================================================================
    private static final Set<String> HIGH_LIQ_PAIRS = new HashSet<>(Arrays.asList(
            ...
            "B-DOGE_USDT", "B-LTC_USDT", "B-BCH_USDT"
    ));
    ...
    // =========================================================================
    // [FIX-9] Curated coin list — meme/low-liquidity removed
    // =========================================================================
    private static final String[] COIN_SYMBOLS = {
        // Tier 1 — top liquidity
        "ETH", "SOL", "BNB", "XRP", "ADA", "DOGE", "LTC", "BCH", "ETC", "XMR",
        // Tier 2 — solid DeFi/L1/L2
        "AVAX", "NEAR", "OP", "ARB", "ATOM", "LINK", "UNI", "AAVE",
        ...
        "JUP", "PYTH", "WIF", "ORDI", "FET", "TAO", "WLD",
        "1000PEPE", "1000SHIB", "1000BONK", "1000FLOKI", "1000SATS",
        "IMX", "MINA", "FLOW", "CHZ", "SAND", "MANA", "GALA", "ENJ",
        "MASK", "BLUR", "ID", "CFX", "AR", "STRK", "ZRO", "ARKM",
        "THETA", "EIGEN", "MORPHO", "KAITO", "USUAL", "LAYER",
        "JTO", "CKB", "TWT", "KSM", "SUSHI", "CAKE", "1INCH",
        "BAT", "ANKR", "XTZ", "ROSE", "CELO", "GAS", "ACT",
        "NMR", "LPT", "STORJ", "ENS", "RPL", "VANA",
        "BOME", "HOT", "LUNA2", "IOST", "FLUX", "ZRX", "LRC",
        "METIS", "GMX", "QNT", "EGLD", "SSV", "FXS", "UMA",
        "XVS", "AKT", "ASTR", "CTSI", "CHR", "C98", "SUN",
        "TNSR", "AERO", "IO", "DEXE", "ARK", "XAI", "DYM",
        "SAGA", "MANTA", "SFP", "AGLD", "ZETA", "RED",
        "LISTA", "WOO", "COW", "VTHO", "LEVER", "SOLV",
        "POWR", "LSK", "MAV", "REI", "ONG", "FORTH",
        "BEL", "GHST", "LOKA", "DIA", "TLM", "FUN",
        "PIXEL", "ARPA", "PNUT", "HIGH", "HOOK",
        "KMNO", "NTRN", "PARTI", "SHELL", "AUCTION", "SWELL",
        "MLN", "STEEM", "MTL", "BICO", "DENT", "RLC",
        "PHB", "DEFI", "COOKIE", "JOE", "BNT", "SCRT",
        "VELODROME", "VIC", "RAYSOL", "BTCDOM", "S", "IP",
        "TRB", "KNC", "KAIA", "BANANA", "POL", "MAGIC",
        "ZK", "NFP", "PEOPLE", "POLYX", "MOVR",
        "OMNI", "CELR", "RVN", "GLM", "HIVE",
        "ILV", "CYBER", "SUPER", "EDU", "PROM",
        "ALT", "WAXP", "ALPHA", "COTI", "USTC",
        "GPS", "SXP", "API3", "ZEN", "AXL", "ACH",
        "GTC", "FIO", "OM", "ETHFI", "SNX", "VIRTUAL",
        "POL", "TAO", "WLD", "ZEC", "PONKE"
    };
    ...
    public static void main(String[] args) {
        ...
        // [FIX-2] Enforce daily limits before doing anything
        if (isDailyLimitHit()) {
        ...
        // [FIX-3] Check position count upfront
        Set<String> active = getActivePositions();
        System.out.printf("Active positions: %d / %d%n", active.size(), MAX_OPEN_POSITIONS);
        if (active.size() >= MAX_OPEN_POSITIONS) {
            System.out.println("Already at max positions — exiting.");
            return;
        }

        // [FIX-4] Fetch live USDT/INR rate once per run
        double usdtInrRate = fetchUsdtInrRate();
        System.out.printf("Live USDT/INR: %.2f%n%n", usdtInrRate);

        for (String pair : COINS_TO_TRADE) {
            try {
                // [FIX-2] Re-check daily limits on each iteration
                if (isDailyLimitHit()) {
                    System.out.println("Daily limit hit — stopping scan.");
                    break;
                }

                // [FIX-3] Re-check position count (may have changed mid-scan)
                ...
                // ── Fetch candles ──────────────────────────────────────────────
                // NOTE: CoinDCX only supports resolutions: '1', '5', '60', '1D'
                // We use 1H (resolution=60) for all signal filters
                // and 5m (resolution=5) only for entry timing confirmation
                ...
                if (raw5m == null || raw5m.length() < 20) {
                    System.out.println("  Insufficient 5m candles — skip");
                    continue;
                }

                // Extract arrays from 1H data (used for all signal logic)
                double[] cl1h = extractCloses(raw1h);
                double[] op1h = extractOpens(raw1h);
                double[] hi1h = extractHighs(raw1h);
                double[] lo1h = extractLows(raw1h);
                double[] vo1h = extractVolumes(raw1h);  // [FIX-7]

                // Extract 5m arrays (entry timing only)
                double[] cl5m = extractCloses(raw5m);
                double[] op5m = extractOpens(raw5m);

                double lastClose = cl1h[cl1h.length - 1];
                double prevClose = cl1h[cl1h.length - 2];
                double prevOpen  = op1h[op1h.length - 2];
                double tickSize  = getTickSize(pair);
                double atr1h     = calcATR(hi1h, lo1h, cl1h, ATR_PERIOD);

                // [FIX-6] ATR volatility filter
                double atrPct = (atr1h / lastClose) * 100.0;
                if (atrPct < ATR_PCT_MIN) {
                    System.out.printf("  Skip — too quiet ATR=%.3f%% (ranging market)%n", atrPct);
                    continue;
                }
                if (atrPct > ATR_PCT_MAX) {
                    System.out.printf("  Skip — too explosive ATR=%.3f%% (uncontrollable risk)%n", atrPct);
                    continue;
                }

                System.out.printf("  Price=%.6f  1H-ATR=%.6f (%.3f%%)  Tick=%.8f%n",
                        lastClose, atr1h, atrPct, tickSize);

                // 5m entry timing candle
                ...
                // ── H1: 1H Macro Trend (EMA-50 on 1H) ─────────────────────────
                double  ema1hMacro = calcEMA(cl1h, EMA_MACRO);
                boolean macroUp    = lastClose > ema1hMacro;
                boolean macroDown  = lastClose < ema1hMacro;
                System.out.printf("  [H1] 1H EMA50=%.6f | %s%n",
                        ema1hMacro, macroUp ? "BULL" : "BEAR");
                if (!macroUp && !macroDown) { System.out.println("  H1 FAIL"); continue; }

                // ── H2: 1H Local Trend (EMA-9 vs EMA-21 on 1H) ────────────────
                double  ema9      = calcEMA(cl1h, EMA_FAST);
                double  ema21     = calcEMA(cl1h, EMA_MID);
                ...
                // ── H3: MACD on 1H ─────────────────────────────────────────────
                double[] mv      = calcMACD(cl1h, MACD_FAST, MACD_SLOW, MACD_SIG);
                ...
                if (trendUp   && !macdBull) { System.out.println("  H3 FAIL"); continue; }
                if (trendDown && !macdBear) { System.out.println("  H3 FAIL"); continue; }

                // ── H4: Supertrend(10,3) on 1H ─────────────────────────────────
                boolean[] stResult   = calcSupertrend(hi1h, lo1h, cl1h, ST_PERIOD, ST_MULTIPLIER);
                ...
                System.out.printf("  [H4] Supertrend=%s%s%n",
                        ...
                        stFlipped ? " *** FLIPPED ***" : "");
                if (trendUp   && !stBull) { System.out.println("  H4 FAIL"); continue; }
                if (trendDown &&  stBull) { System.out.println("  H4 FAIL"); continue; }

                System.out.println("  All 4 HARD filters PASSED.");

                // ── S1: RSI — non-overlapping zones [FIX-5] ────────────────────
                double  rsi        = calcRSI(cl1h, RSI_PERIOD);
                ...
                double avgVol  = 0;
                int    vbars   = Math.min(10, vo1h.length - 2);
                for (int i = vo1h.length - 2 - vbars; i < vo1h.length - 2; i++) avgVol += vo1h[i];
                avgVol /= vbars;
                double  lastVol    = vo1h[vo1h.length - 2];
                boolean softVolume = avgVol > 0 && lastVol > VOLUME_SPIKE_MULT * avgVol;
                System.out.printf("  [S3] Vol=%.0f avg=%.0f (%.1fx) -> %s%n",
                        lastVol, avgVol,
                        avgVol > 0 ? lastVol / avgVol : 0.0,
                        softVolume ? "PASS" : "fail");

                // Need 2 of 3 soft filters [FIX-7]
                int softPass = (softRsi ? 1 : 0) + (softCandle ? 1 : 0) + (softVolume ? 1 : 0);
                System.out.printf("  SOFT: %d/3 passed (need 2)%n", softPass);
                if (softPass < 2) {
                    System.out.println("  SOFT FAIL — skip");
                    continue;
                }

                // ── Entry timing: 5m candle must match direction ────────────────
                ...
                System.out.printf("  [5m] Entry candle %s -> %s%n",
                        ...
                        entry5mOk ? "PASS" : "WAIT");
                if (!entry5mOk) {
                    System.out.println("  5m entry not ideal — skip");
                    continue;
                }

                // ── Price extension check ───────────────────────────────────────
                double distFromEma9 = Math.abs(lastClose - ema9);
                if (distFromEma9 > 0.8 * atr1h) {
                    System.out.printf("  Skip — price extended from EMA9 (dist=%.6f > 0.8xATR=%.6f)%n",
                            distFromEma9, 0.8 * atr1h);
                    continue;
                }

                // ── [FIX-8] Pre-calculate SL/TP and validate R:R BEFORE ordering
                double currentPrice = getLastPrice(pair);
                if (currentPrice <= 0) { System.out.println("  Invalid price — skip"); continue; }

                String side = trendUp ? "buy" : "sell";
                double[] slTpPre = calcSlTp(side, currentPrice, hi1h, lo1h, atr1h, tickSize);
                double preSlPrice = slTpPre[0];
                double preTpPrice = slTpPre[1];
                double actualRR   = Math.abs(preTpPrice - currentPrice)
                                  / Math.abs(currentPrice - preSlPrice);

                if (actualRR < MIN_RR_REQUIRED) {
                    System.out.printf("  Skip — actual R:R %.2f < minimum %.1f%n",
                            actualRR, MIN_RR_REQUIRED);
                    continue;
                }

                System.out.printf("  Pre-check R:R=1:%.2f — OK%n", actualRR);
                System.out.printf("  Pre SL=%.6f | Pre TP=%.6f%n", preSlPrice, preTpPrice);

                // ── All filters passed ──────────────────────────────────────────
                System.out.println("  >>> ALL FILTERS PASSED -> " + side.toUpperCase()
                        + " " + pair + (stFlipped ? " [ST FLIP — HIGH CONF]" : ""));

                // Determine margin (cautious mode after profit target)
                double margin = (dailyPnl >= DAILY_PROFIT_TARGET) ? CAUTIOUS_MARGIN : MAX_MARGIN;
                int    lev    = getLeverage(pair);
                double qty    = calcQuantity(currentPrice, pair, usdtInrRate, margin, lev);
                if (qty <= 0) { System.out.println("  Invalid qty — skip"); continue; }

                System.out.printf("  Placing %s | price=%.6f | qty=%.4f | lev=%dx | margin=%.0f INR%n",
                        side.toUpperCase(), currentPrice, qty, lev, margin);
                ...
                System.out.println("  Order placed! id=" + resp.optString("id"));
                lastTradeTime.put(pair, System.currentTimeMillis());

                // ── Confirm entry price ─────────────────────────────────────────
                double entry = getEntryPrice(pair, resp.optString("id"));
                if (entry <= 0) {
                ...
                // ── Final SL/TP using actual fill price ─────────────────────────
                double[] slTpFinal = calcSlTp(side, entry, hi1h, lo1h, atr1h, tickSize);
                double slPrice = slTpFinal[0];
                double tpPrice = slTpFinal[1];
                double finalRR = Math.abs(tpPrice - entry) / Math.abs(entry - slPrice);

                double slPct = Math.abs(entry - slPrice) / entry * 100;
                double tpPct = Math.abs(tpPrice - entry) / entry * 100;
                System.out.printf("  Final SL=%.6f (%.2f%%) | TP=%.6f (%.2f%%) | R:R=1:%.2f%n",
                        slPrice, slPct, tpPrice, tpPct, finalRR);
                ...
                // [FIX-10] Log trade
                logTrade(pair, side, entry, slPrice, tpPrice, qty, lev, finalRR);
    ...
    // =========================================================================
    // [FIX-2] Daily P&L Protection — enforced
    // =========================================================================
    private static boolean isDailyLimitHit() {
        long today = LocalDate.now().toEpochDay();
        if (today != lastResetDay) {
            dailyPnl  = 0; dailyPeak = 0; dailyLoss = 0;
            lastResetDay = today;
            System.out.println("Daily P&L counters reset.");
        }
        ...
        if (dailyPeak > 0 && (dailyPeak - dailyPnl) >= DAILY_DRAWDOWN_LIMIT) {
            System.out.printf("DAILY DRAWDOWN LIMIT HIT (drop=%.2f from peak=%.2f) — stopped%n",
                    dailyPeak - dailyPnl, dailyPeak);
            return true;
        }
        return false;
    }
    ...
    /**
     * Call this externally (or via webhook/cron) when a position closes.
     * Updates daily P&L tracking so limits work correctly.
     */
    public static void updateDailyPnl(double pnl) {
        dailyPnl  += pnl;
        dailyPeak  = Math.max(dailyPeak, dailyPnl);
        if (pnl < 0) dailyLoss += Math.abs(pnl);
        System.out.printf("P&L updated: daily=%.2f peak=%.2f loss=%.2f%n",
                dailyPnl, dailyPeak, dailyLoss);
    }

    // =========================================================================
    // [FIX-4] Live USDT/INR rate
    // =========================================================================
    private static double fetchUsdtInrRate() {
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (rate > 50 && rate < 200) {
                    System.out.printf("Live USDT/INR rate fetched: %.2f%n", rate);
                    return rate;
                }
            }
        } catch (Exception e) {
            System.err.println("Rate fetch failed — using fallback 85.0: " + e.getMessage());
        }
        return 85.0; // conservative fallback (real rate ~84-86 as of 2026)
    }

    // =========================================================================
    // SL/TP Calculation — returns [slPrice, tpPrice]
    // =========================================================================
    private static double[] calcSlTp(String side, double entry,
        ...
        if ("buy".equalsIgnoreCase(side)) {
            double swLow = swingLow(lo, 30);
            double rawSL = swLow - (NOISE_BUFFER * atr) - (SL_SWING_BUFFER * atr);
            double minSL = entry - SL_MIN_ATR * atr;
            double maxSL = entry - SL_MAX_ATR * atr;
            sl = Math.max(Math.min(rawSL, minSL), maxSL);
            tp = entry + RR * (entry - sl);
            System.out.printf("  SwingLow=%.6f RawSL=%.6f ClampedSL=%.6f%n", swLow, rawSL, sl);
        } else {
            double swHigh = swingHigh(hi, 30);
            double rawSL  = swHigh + (NOISE_BUFFER * atr) + (SL_SWING_BUFFER * atr);
            double minSL  = entry + SL_MIN_ATR * atr;
            double maxSL  = entry + SL_MAX_ATR * atr;
            sl = Math.min(Math.max(rawSL, minSL), maxSL);
            tp = entry - RR * (sl - entry);
            System.out.printf("  SwingHigh=%.6f RawSL=%.6f ClampedSL=%.6f%n", swHigh, rawSL, sl);
        }
        return new double[]{ roundToTick(sl, tickSize), roundToTick(tp, tickSize) };
    }
    ...
    private static void logTrade(String pair, String side, double entry,
        ...
        try (FileWriter fw = new FileWriter("trades.log", true)) {
            fw.write(String.format("%s,%s,%s,entry=%.6f,SL=%.6f,TP=%.6f,qty=%.4f,lev=%dx,RR=1:%.2f%n",
    ...
    // [FIX-7] Volume extraction — confirmed field name "volume" from API docs
    private static double[] extractVolumes(JSONArray a) {
        double[] o = new double[a.length()];
        for (int i = 0; i < a.length(); i++) {
            JSONObject c = a.getJSONObject(i);
            o[i] = c.has("volume") ? c.getDouble("volume")
                 : c.has("vol")    ? c.getDouble("vol") : 0.0;
        }
        return o;
    }
     ...
     * IMPORTANT: CoinDCX only supports resolutions: '1', '5', '60', '1D'
     * '15' is NOT a valid resolution per official API docs.
     */
    private static JSONArray getCandlestickData(String pair, String resolution, int count) {
        try {
            ...
            switch (resolution) {
                ...
                case "1D": minsPerBar = 1440; break;
                default:
                    System.err.println("  WARNING: invalid resolution '" + resolution
                        + "'. Valid: 1, 5, 60, 1D");
                    minsPerBar = 60;
                    break;
            }
    ...
    private static JSONObject findPosition(String pair) throws Exception {
        ...
        body.put("page", "1");
        body.put("size", "20");
        // Per docs: margin_currency_short_name is an Array
    ...
    // [FIX-4] Updated to accept live rate, margin, and leverage
    private static double calcQuantity(double price, String pair,
                                        double usdtInrRate, double margin, int lev) {
        double qty = (margin * lev) / (price * usdtInrRate);
     ...
     *
     * API CORRECTIONS from docs:
     *   - take_profit order_type: ONLY "take_profit_market" is supported
     *   - stop_loss order_type:   ONLY "stop_market" is supported
     *   - limit_price: NOT supported — removed from payload
     *   - stop_price is the TRIGGER price for both TP and SL
     */
    public static void setTpSl(String posId, double tp, double sl, String pair) {
        try {
            ...
            // Per docs: only stop_price and order_type are needed
            // limit_price is NOT supported (docs say "Ignore this for now")
            JSONObject tpObj = new JSONObject();
            tpObj.put("stop_price", String.valueOf(rtp));  // docs use String for price
            tpObj.put("order_type", "take_profit_market"); // only supported type

            JSONObject slObj = new JSONObject();
            slObj.put("stop_price", String.valueOf(rsl));  // docs use String for price
            slObj.put("order_type", "stop_market");        // only supported type
            ...
            // Per docs: response has "success" field per TP/SL entry
            JSONObject r = new JSONObject(resp);
            boolean tpOk = true, slOk = true;
            if (r.has("take_profit")) {
                JSONObject tpResp = r.getJSONObject("take_profit");
                tpOk = tpResp.optBoolean("success", true);
                if (!tpOk) System.out.println("  TP error: " + tpResp.optString("error"));
            }
            if (r.has("stop_loss")) {
                JSONObject slResp = r.getJSONObject("stop_loss");
                // stop_loss success is implied by status field
                String status = slResp.optString("status", "");
                slOk = "untriggered".equals(status) || !status.isEmpty();
            }
            if (tpOk && slOk) {
                System.out.printf("  TP/SL set! TP=%.6f SL=%.6f%n", rtp, rsl);
            } else {
                System.out.println("  TP/SL partial: " + resp);
            }
        } catch (Exception e) {
    ...
    public static String getPositionId(String pair) {
        try {
            ...
            // Per docs: position id is FIXED for a pair — reuse across sessions
    ...
    private static Set<String> getActivePositions() {
        ...
        try {
            ...
            for (int i = 0; i < arr.length(); i++) {
                ...
                double    activeQ  = p.optDouble("active_pos", 0);
                // Per docs: active_pos is NEGATIVE for short positions
                boolean isActive = Math.abs(activeQ) > 0
