...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // (unchanged from the version you said was giving correct long/short calls)
    // =========================================================================
    ...
    // ---- Entry (5M) filter thresholds — strict all-AND, unchanged ----
    ...
    // ---- SL/TP sizing — FIXED. Computed once at entry, confirmed on the
    // exchange, and never touched again (no breakeven lock, no trailing).
    // Whichever order — take_profit_market or stop_market — fills first,
    // closes the trade. That is the entire exit mechanism now. ----
    ...
    // =========================================================================
    // DIRECTION (1H & 30M) — plain EMA9/EMA21 crossover. Deliberately simple:
    // this is just the top-level trend bias, the 15M setup and 5M entry do
    // the real filtering.
    // =========================================================================
    ...
    // =========================================================================
    // SETUP (15M) — EMA cross + Supertrend colour + EMA9 slope (real
    // momentum, not just a static snapshot). Also returns the Supertrend
    // band levels so fixed SL sizing can use a structural level instead of
    // relying on 5M swing alone.
    // =========================================================================
    ...
    // =========================================================================
    // ENTRY (5M) — strict all-AND trigger. Every one of the six conditions
    // (pullback, directional candle, not-a-doji, volume, RSI, VWAP) must be
    // true simultaneously. This is the tight filter you said was working
    // well for direction/entry quality.
    // =========================================================================
    ...
    // =========================================================================
    // FIXED SL/TP — computed once at entry (or by the safety sweep), set on
    // the exchange, and NEVER adjusted afterward. No breakeven lock, no
    // trailing. SL uses the structural level (5M swing extreme OR the 15M
    // Supertrend band, whichever is closer/tighter to price) plus an ATR
    // buffer, clamped to a hard max percent. TP is entry ± RR_TARGET x risk.
    // =========================================================================
    ...
    // =========================================================================
    // Orchestrator — simple loop: scan for entries (fixed SL/TP set once),
    // then safety-sweep any open position that ended up without protection.
    // No trailing poll loop, no trail-state file.
    // =========================================================================
    public static void main(String[] args) {
        System.out.println("=== Scalp bot starting (fixed SL/TP, no trailing/breakeven) ===");
        initInstrumentCache();
