...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * CoinDCX Futures Trader — DAILY / 4H / 1H FRAMEWORK REWRITE (v10)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Naya architecture (jo aapne bheja hua doc/points kehta hai):
 *
 *   DAILY   = Macro Filter    -> 200-EMA (major trend) + 9/21-EMA (trend strength)
 *   4-HOUR  = Primary Signal  -> 20/50-EMA cross + RSI(14) + MACD(12,26,9) + Volume
 *   1-HOUR  = Entry Timing    -> 9/21-EMA cross + pullback to 20-EMA (within 1%)
 *
 *   SL  = Entry ∓ 1.5×ATR(4H)
 *   TP1 = Entry ± 2×ATR(4H)  -> CLOSE 50% of position (reduce-only limit order)
 *   TP2 = remaining 50%      -> TRAILING stop, 2×ATR(4H) from peak/trough price
 *   Breakeven = move SL to entry once price has moved +/-5% in your favor
 *
 * IMPORTANT ASSUMPTIONS (verify against your CoinDCX API docs before running):
 *   1) Candlestick endpoint accepts resolution="1D" for daily candles.
 *   2) Candle JSON objects contain a "volume" field.
 *   3) Order creation accepts a boolean "reduce_only" field (used for the
 *      TP1 partial-close order). If your account/API version doesn't
 *      support this, replace placeReduceOnlyOrder() with your exchange's
 *      equivalent (e.g. a dedicated reduce-only endpoint).
 *
 * STATE PERSISTENCE:
 *   Cooldowns aur per-position TP1/breakeven/trailing state ab EK hi file
 *   "bot_state.json" me persist hote hain (GitHub Actions artifact ke through
 *   save/restore karna — jaise aapka last_trade_state.json pattern pehle se
 *   tha). Isके bina trailing stop aur TP1-filled tracking har run pe reset
 *   ho jayegi.
 * ═══════════════════════════════════════════════════════════════════════════
 */
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    private static final double MAX_MARGIN = 600.0;   // per-trade margin cap (INR) — size this to reflect your 1-2% risk rule
    ...
    private static final long POSITION_ID_RETRY_DELAY_MS = 1000L;

    private static final int  TPSL_MAX_RETRIES    = 5;
    private static final long TPSL_RETRY_DELAY_MS = 3000L;
    ...
    private static final String STATE_FILE = "bot_state.json";

    // ── Indicator periods (per framework doc) ────────────────────────────────
    private static final int EMA_FAST   = 9;
    private static final int EMA_MID    = 21;
    private static final int EMA20_4H   = 20;
    private static final int EMA50_4H   = 50;
    private static final int EMA20_1H   = 20;
    private static final int EMA_DAILY_MAJOR = 200;
    ...
    private static final double TP1_ATR_MULT      = 2.0;
    private static final double TP2_TRAIL_ATR_MULT = 2.0;
    private static final double FAR_TP_ATR_MULT   = 8.0; // exchange-side safety-net TP, in case trailing management stalls
    private static final double TP1_CLOSE_FRACTION = 0.5;
    private static final double BREAKEVEN_TRIGGER_PCT = 5.0;

    private static final double LIMIT_ORDER_BUFFER_PCT = 0.001; // 0.1%
    private static final int    MIN_TICKS_AWAY = 2;

    // ── Candle fetch counts ───────────────────────────────────────────────────
    private static final int CANDLE_DAILY_COUNT = 260;   // needs 200-EMA + buffer
    private static final int CANDLE_1H_COUNT    = 150;
    private static final int NEED_4H_CANDLES    = 150;   // after aggregation
    private static final int HTF_1H_FETCH_COUNT = NEED_4H_CANDLES * 4 + 20; // for 4H aggregation + 1H reuse

    private static final Map<String, JSONObject> instrumentCache = new ConcurrentHashMap<>();
    private static long lastCacheUpdate = 0;

    // ── Runtime state (loaded from / saved to STATE_FILE) ────────────────────
    private static final Map<String, Long> lastTradeTime = new ConcurrentHashMap<>();
    private static final Map<String, PositionState> positionStates = new ConcurrentHashMap<>();

    // =========================================================================
    // Coin list — unchanged from prior version
    // =========================================================================
    ...
    // =========================================================================
    // Per-timeframe result structs
    // =========================================================================
    private static class DailyResult {
        boolean valid, bullish, bearish, choppy;
        double price, ema200, ema9, ema21;
    }

    private static class FourHResult {
        boolean valid;
        double[] hi, lo, cl, vol;
        double atr;
        boolean goldenCross, deathCross, rsiLongOk, rsiShortOk, macdBullCross, macdBearCross, volOk;
    }

    private static class OneHResult {
        boolean valid;
        double price, ema20;
        boolean bullishCross, bearishCross, pullbackOk;
    }

    // Per-position management state, persisted across runs
    private static class PositionState {
        ...
        double tp1Price;
        double tp1Qty;
        boolean tp1Filled = false;
        boolean breakevenApplied = false;
        double trailingSL;      // current enforced SL (monotonic — only improves)
        double peakPrice;       // highest price since entry (long) / lowest (short)
        String tp1OrderId;
        ...
        JSONObject toJson() {
            ...
            o.put("atrAtEntry", atrAtEntry);
            o.put("originalQty", originalQty);
            o.put("tp1Price", tp1Price);
            o.put("tp1Qty", tp1Qty);
            o.put("tp1Filled", tp1Filled);
            o.put("breakevenApplied", breakevenApplied);
            o.put("trailingSL", trailingSL);
            o.put("peakPrice", peakPrice);
            if (tp1OrderId != null) o.put("tp1OrderId", tp1OrderId);
            return o;
        }

        static PositionState fromJson(JSONObject o) {
            PositionState p = new PositionState();
            p.side = o.optString("side", "buy");
            p.entryPrice = o.optDouble("entryPrice", 0);
            p.atrAtEntry = o.optDouble("atrAtEntry", 0);
            p.originalQty = o.optDouble("originalQty", 0);
            p.tp1Price = o.optDouble("tp1Price", 0);
            p.tp1Qty = o.optDouble("tp1Qty", 0);
            p.tp1Filled = o.optBoolean("tp1Filled", false);
            p.breakevenApplied = o.optBoolean("breakevenApplied", false);
            p.trailingSL = o.optDouble("trailingSL", 0);
            p.peakPrice = o.optDouble("peakPrice", 0);
            p.tp1OrderId = o.has("tp1OrderId") ? o.getString("tp1OrderId") : null;
            return p;
        }
    }

    // =========================================================================
    // ANALYSIS — Daily macro filter
    // =========================================================================
    private static DailyResult analyzeDaily(JSONArray candles) {
        DailyResult r = new DailyResult();
        if (candles == null || candles.length() < EMA_DAILY_MAJOR + 5) { r.valid = false; return r; }
        double[] cl = extractCloses(candles);
        r.ema200 = calcEMA(cl, EMA_DAILY_MAJOR);
        r.ema9   = calcEMA(cl, EMA_FAST);
        r.ema21  = calcEMA(cl, EMA_MID);
        r.price  = cl[cl.length - 1];
        r.valid  = true;
        r.choppy = Math.abs(r.price - r.ema200) / r.price * 100 < DAILY_CHOP_PCT;
        r.bullish = !r.choppy && r.price > r.ema200 && r.price > r.ema9 && r.price > r.ema21;
        r.bearish = !r.choppy && r.price < r.ema200 && r.price < r.ema9 && r.price < r.ema21;
        return r;
    }

    // =========================================================================
    // ANALYSIS — 4H primary signal
    // =========================================================================
    private static FourHResult analyze4H(JSONArray candles) {
        FourHResult r = new FourHResult();
        int minNeeded = Math.max(EMA50_4H, MACD_SLOW + MACD_SIGNAL) + VOL_AVG_PERIOD + 10;
        if (candles == null || candles.length() < minNeeded) { r.valid = false; return r; }
        double[] cl  = extractCloses(candles);
        double[] hi  = extractHighs(candles);
        double[] lo  = extractLows(candles);
        double[] vol = extractVolumes(candles);
        r.hi = hi; r.lo = lo; r.cl = cl; r.vol = vol;
    ...
    // =========================================================================
    // ANALYSIS — 1H entry timing
    // =========================================================================
    private static OneHResult analyze1H(JSONArray candles) {
        OneHResult r = new OneHResult();
        if (candles == null || candles.length() < EMA_MID + 5) { r.valid = false; return r; }
        double[] cl = extractCloses(candles);
        r.price = cl[cl.length - 1];
        double[] ema9s  = calcEMASeries(cl, EMA_FAST);
        double[] ema21s = calcEMASeries(cl, EMA_MID);
        double[] ema20s = calcEMASeries(cl, EMA20_1H);
        r.ema20 = ema20s[ema20s.length - 1];
        r.bullishCross = crossedAbove(ema9s, ema21s, CROSS_LOOKBACK_1H);
        r.bearishCross = crossedBelow(ema9s, ema21s, CROSS_LOOKBACK_1H);
        r.pullbackOk = Math.abs(r.price - r.ema20) / r.price * 100 <= PULLBACK_PCT_1H;
        r.valid = true;
        return r;
    }
    ...
    private static double[] calcEMASeries(double[] d, int period) {
        ...
        if (n < period) { Arrays.fill(ema, n > 0 ? d[n - 1] : 0); return ema; }
        double k = 2.0 / (period + 1);
        double sum = 0;
        for (int i = 0; i < period; i++) sum += d[i];
        double prev = sum / period;
        for (int i = 0; i < period; i++) ema[i] = prev;
        for (int i = period; i < n; i++) { prev = d[i] * k + prev * (1 - k); ema[i] = prev; }
        return ema;
    }

    private static double calcEMA(double[] d, int period) {
        double[] s = calcEMASeries(d, period);
        return s[s.length - 1];
    }

    private static double[] calcRSISeries(double[] cl, int period) {
        int n = cl.length;
        double[] rsi = new double[n];
        if (n < period + 1) { Arrays.fill(rsi, 50); return rsi; }
        double gainSum = 0, lossSum = 0;
        for (int i = 1; i <= period; i++) {
            double diff = cl[i] - cl[i - 1];
            if (diff >= 0) gainSum += diff; else lossSum += -diff;
        }
        double avgGain = gainSum / period, avgLoss = lossSum / period;
        for (int i = 0; i <= period; i++) rsi[i] = avgLoss == 0 ? 100 : 100 - (100 / (1 + avgGain / avgLoss));
    ...
    private static double[][] calcMACD(double[] cl, int fast, int slow, int signalP) {
        double[] emaFast = calcEMASeries(cl, fast);
        double[] emaSlow = calcEMASeries(cl, slow);
        int n = cl.length;
        double[] macdLine = new double[n];
        for (int i = 0; i < n; i++) macdLine[i] = emaFast[i] - emaSlow[i];
        double[] signalLine = calcEMASeries(macdLine, signalP);
    ...
    private static boolean crossedAbove(double[] a, double[] b, int lookback) {
        int n = a.length;
    ...
    private static boolean crossedBelow(double[] a, double[] b, int lookback) {
        int n = a.length;
        for (int i = Math.max(1, n - lookback); i < n; i++) {
            if (a[i - 1] >= b[i - 1] && a[i] < b[i]) return true;
        }
    ...
    private static double calcATR(double[] hi, double[] lo, double[] cl, int period) {
        double[] s = calcATRSeries(hi, lo, cl, period);
        return s[s.length - 1];
    }
    ...
    public static void main(String[] args) {
        ...
        // Manage existing positions FIRST (TP1 detection, breakeven, trailing)
        managePositions(active);

        if (active.size() >= MAX_OPEN_POSITIONS) {
            System.out.println("MAX_OPEN_POSITIONS (" + MAX_OPEN_POSITIONS + ") reached — skipping new-entry scan.");
            saveState();
            return;
        }
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                if (System.currentTimeMillis() - lastTrade < COOLDOWN_MS) continue;

                System.out.println("\n==== " + pair + " ====");

                JSONArray rawDaily      = getCandlestickData(pair, "1D", CANDLE_DAILY_COUNT);
                JSONArray raw1hExtended = getCandlestickData(pair, "60", HTF_1H_FETCH_COUNT);
                if (rawDaily == null || raw1hExtended == null) {
                    System.out.println("  Insufficient candle data — skip"); continue;
                }
                JSONArray raw4h = aggregateCandles(raw1hExtended, 4);
                JSONArray raw1h = lastN(raw1hExtended, CANDLE_1H_COUNT);

                // ── STEP 1 — Daily macro filter ───────────────────────────
                DailyResult d = analyzeDaily(rawDaily);
                if (!d.valid) { System.out.println("  [Daily] insufficient data — skip"); continue; }
                System.out.printf("  [Daily] price=%.6f EMA200=%.6f EMA9=%.6f EMA21=%.6f choppy=%s -> %s%n",
                        d.price, d.ema200, d.ema9, d.ema21, d.choppy,
                        d.bullish ? "BULLISH" : d.bearish ? "BEARISH" : "NO TREND");
                if (!d.bullish && !d.bearish) { System.out.println("  Daily FAIL — no clean macro trend — skip"); continue; }

                // ── STEP 2 — 4H primary signal ────────────────────────────
                FourHResult f = analyze4H(raw4h);
                if (!f.valid) { System.out.println("  [4H] insufficient data — skip"); continue; }

                boolean longSignal4h  = f.goldenCross && f.rsiLongOk && f.macdBullCross && f.volOk;
                boolean shortSignal4h = f.deathCross && f.rsiShortOk && f.macdBearCross && f.volOk;
                System.out.printf("  [4H] goldenCross=%s deathCross=%s rsiLongOk=%s rsiShortOk=%s macdBullCross=%s macdBearCross=%s volOk=%s%n",
                        f.goldenCross, f.deathCross, f.rsiLongOk, f.rsiShortOk, f.macdBullCross, f.macdBearCross, f.volOk);

                boolean trendUp;
                if (d.bullish && longSignal4h) trendUp = true;
                else if (d.bearish && shortSignal4h) trendUp = false;
                else { System.out.println("  4H FAIL — primary signal doesn't confirm Daily trend — skip"); continue; }

                // ── STEP 3 — 1H entry timing ──────────────────────────────
                OneHResult h = analyze1H(raw1h);
                if (!h.valid) { System.out.println("  [1H] insufficient data — skip"); continue; }
                boolean entryOk = trendUp ? (h.bullishCross && h.pullbackOk) : (h.bearishCross && h.pullbackOk);
                System.out.printf("  [1H] bullishCross=%s bearishCross=%s pullbackOk=%s -> %s%n",
                        h.bullishCross, h.bearishCross, h.pullbackOk, entryOk ? "TRIGGER" : "WAIT");
                if (!entryOk) { System.out.println("  1H FAIL — no entry trigger yet — skip"); continue; }
                ...
                JSONObject resp = placeFuturesOrder(side, pair, qty, LEVERAGE, "email_notification", "isolated", "INR", currentPrice, false);
                if (resp == null || !resp.has("id")) { System.out.println("  Order failed: " + resp); continue; }
                System.out.println("  Order placed! id=" + resp.getString("id"));
                lastTradeTime.put(pair, System.currentTimeMillis());

                double entry = getEntryPrice(pair);
                if (entry <= 0) {
                    System.out.println("  Could not confirm entry — TP1/SL/trailing will be set up by managePositions() next run");
                    active.add(pair);
                    continue;
                }
                System.out.printf("  Entry confirmed: %.6f%n", entry);

                // ── SL / TP1 / TP2-trailing setup, all based on 4H ATR ─────
                double atr4h = f.atr;
                double sl, tp1, farTp;
                if (trendUp) {
                    sl    = entry - SL_ATR_MULT * atr4h;
                    tp1   = entry + TP1_ATR_MULT * atr4h;
                    farTp = entry + FAR_TP_ATR_MULT * atr4h;
                } else {
                    sl    = entry + SL_ATR_MULT * atr4h;
                    tp1   = entry - TP1_ATR_MULT * atr4h;
                    farTp = entry - FAR_TP_ATR_MULT * atr4h;
                }
                sl = clampMinDistance(sl, entry, tickSize, trendUp, false);
                tp1 = clampMinDistance(tp1, entry, tickSize, trendUp, true);
                sl = roundToTick(sl, tickSize);
                tp1 = roundToTick(tp1, tickSize);
                farTp = roundToTick(farTp, tickSize);

                System.out.printf("  SL=%.6f | TP1(50%%)=%.6f | far-safety-TP=%.6f | ATR(4H)=%.6f%n", sl, tp1, farTp, atr4h);

                String posId = getPositionIdWithRetry(pair);
                if (posId == null) {
                    System.out.println("  Position ID not found after retries — safety pass will retry next run");
                    active.add(pair);
                    continue;
                }
                setTpSlWithRetry(posId, farTp, sl, pair);

                // TP1: reduce-only partial close order for 50% of qty
                double tp1Qty = INTEGER_QTY_PAIRS.contains(pair)
                        ? Math.floor(qty * TP1_CLOSE_FRACTION)
                        : Math.floor(qty * TP1_CLOSE_FRACTION * 100) / 100.0;
                if (tp1Qty <= 0) tp1Qty = qty; // tiny qty pairs: TP1 closes everything, trailing leg skipped
                String tp1OrderSide = trendUp ? "sell" : "buy";
                JSONObject tp1Resp = placeReduceOnlyOrder(tp1OrderSide, pair, tp1Qty, tp1);
                String tp1OrderId = (tp1Resp != null && tp1Resp.has("id")) ? tp1Resp.getString("id") : null;
                if (tp1OrderId == null) System.out.println("  WARNING: TP1 reduce-only order failed — check reduce_only support on your account/API");
                ...
                ps.tp1Qty = tp1Qty;
                ps.tp1Filled = false;
                ps.breakevenApplied = false;
                ps.trailingSL = sl;
                ps.peakPrice = entry;
                ps.tp1OrderId = tp1OrderId;
                positionStates.put(pair, ps);
    ...
    // Ensures SL/TP1 stay a minimum tick distance from entry so the exchange
    // doesn't silently reject the order.
    private static double clampMinDistance(double price, double entry, double tick, boolean isLong, boolean isTp) {
        double minGap = tick * MIN_TICKS_AWAY;
        if (isLong) {
            if (isTp) { if (price - entry < minGap) price = entry + minGap; }
            else      { if (entry - price < minGap) price = entry - minGap; }
        } else {
            if (isTp) { if (entry - price < minGap) price = entry - minGap; }
            else      { if (price - entry < minGap) price = entry + minGap; }
        }
        return price;
    }

    // =========================================================================
    // POSITION MANAGEMENT — TP1 detection, breakeven move, TP2 trailing stop
    // =========================================================================
    private static void managePositions(Set<String> stillOpen) {
        try {
            for (String pair : new ArrayList<>(positionStates.keySet())) {
                if (!stillOpen.contains(pair)) {
                    System.out.println("  [MANAGE] " + pair + " no longer open — clearing saved state");
                    positionStates.remove(pair);
                    continue;
                }
                PositionState ps = positionStates.get(pair);
                JSONObject pos = findPosition(pair);
                if (pos == null) continue;

                boolean isLong = ps.side.equalsIgnoreCase("buy");
                double currentQty = Math.abs(pos.optDouble("active_pos", 0));
                double currentPrice = getLastPrice(pair);
                if (currentPrice <= 0) continue;

                // Update peak/trough since entry
                if (isLong) ps.peakPrice = Math.max(ps.peakPrice, currentPrice);
                else        ps.peakPrice = Math.min(ps.peakPrice == 0 ? currentPrice : ps.peakPrice, currentPrice);

                // TP1 detection — position size shrank vs original -> TP1 reduce-only order filled
                if (!ps.tp1Filled && ps.originalQty > 0 && currentQty > 0 && currentQty < ps.originalQty * 0.9) {
                    ps.tp1Filled = true;
                    System.out.println("  [MANAGE] " + pair + " TP1 appears filled (qty " + ps.originalQty + " -> " + currentQty + ")");
                }

                // Breakeven check — +/-5% move in favor
                double movePct = isLong
                        ? (currentPrice - ps.entryPrice) / ps.entryPrice * 100
                        : (ps.entryPrice - currentPrice) / ps.entryPrice * 100;
                if (!ps.breakevenApplied && movePct >= BREAKEVEN_TRIGGER_PCT) {
                    ps.breakevenApplied = true;
                    ps.trailingSL = isLong ? Math.max(ps.trailingSL, ps.entryPrice) : Math.min(ps.trailingSL, ps.entryPrice);
                    System.out.println("  [MANAGE] " + pair + " breakeven triggered (+" + String.format("%.2f", movePct) + "%) -> SL=" + ps.trailingSL);
                }

                // TP2 trailing — only actively trail once TP1 has closed the first half
                if (ps.tp1Filled) {
                    double candidate = isLong
                            ? ps.peakPrice - TP2_TRAIL_ATR_MULT * ps.atrAtEntry
                            : ps.peakPrice + TP2_TRAIL_ATR_MULT * ps.atrAtEntry;
                    boolean improved = isLong ? candidate > ps.trailingSL : candidate < ps.trailingSL;
                    if (improved) {
                        ps.trailingSL = candidate;
                        System.out.println("  [MANAGE] " + pair + " trailing SL updated -> " + ps.trailingSL);
                    }
                }

                // Push updated SL to exchange (keep existing far-safety TP as-is)
                double tick = getTickSize(pair);
                double newSl = roundToTick(ps.trailingSL, tick);
                double storedTp = pos.optDouble("take_profit_trigger", 0);
                String posId = pos.optString("id", null);
                if (posId != null) {
                    setTpSlWithRetry(posId, storedTp > 0 ? storedTp : newSl + (isLong ? 1 : -1) * FAR_TP_ATR_MULT * ps.atrAtEntry, newSl, pair);
                }
            }
        } catch (Exception e) {
            System.err.println("managePositions: " + e.getMessage());
        }
    }

    // =========================================================================
    // TP/SL retry-and-confirm setter
    // =========================================================================
    private static boolean setTpSlWithRetry(String posId, double tp, double sl, String pair) {
        for (int attempt = 1; attempt <= TPSL_MAX_RETRIES; attempt++) {
            ...
            try { TimeUnit.MILLISECONDS.sleep(TPSL_RETRY_DELAY_MS); } catch (InterruptedException ignored) {}
            try {
                JSONObject pos = findPosition(pair);
                if (pos != null && pos.optDouble("stop_loss_trigger", 0) > 0) {
                    System.out.println("  TP/SL confirmed set on attempt " + attempt + " for " + pair);
                    return true;
                }
            } catch (Exception ignored) {}
            System.out.println("  TP/SL not confirmed yet (attempt " + attempt + "/" + TPSL_MAX_RETRIES + ") for " + pair);
        }
    ...
    private static JSONArray getCandlestickData(String pair, String resolution, int count) {
        try {
            ...
            switch (resolution) {
                ...
                case "1D":  minsPerBar = 1440; break; // NOTE: verify "1D" is accepted by your CoinDCX API version
    ...
    private static double getEntryPrice(String pair) throws Exception {
    ...
    private static String getPositionIdWithRetry(String pair) {
        for (int i = 0; i < POSITION_ID_MAX_RETRIES; i++) {
            String id = getPositionId(pair);
            if (id != null) return id;
            try { TimeUnit.MILLISECONDS.sleep(POSITION_ID_RETRY_DELAY_MS); } catch (InterruptedException ignored) {}
        }
        return null;
    }
    ...
    private static double calcQuantity(double price, String pair) {
        ...
        return Math.max(INTEGER_QTY_PAIRS.contains(pair) ? Math.floor(qty) : Math.floor(qty * 100) / 100.0, 0);
    }
    ...
    public static JSONObject placeFuturesOrder(String side, String pair, double qty, int lev, String notif,
                                                String marginType, String marginCcy, double currentPrice, boolean reduceOnly) {
        try {
            ...
            if (reduceOnly) order.put("reduce_only", true); // NOTE: verify this field name against your API docs
    ...
    // TP1's 50% partial close — a plain limit order flagged reduce_only so it
    // can only shrink the existing position, never flip or add to it.
    private static JSONObject placeReduceOnlyOrder(String side, String pair, double qty, double limitPrice) {
        double tick = getTickSize(pair);
        limitPrice = roundToTick(limitPrice, tick);
        try {
            JSONObject order = new JSONObject();
            ...
            order.put("hidden", false);
            order.put("post_only", false);
            order.put("reduce_only", true); // NOTE: verify this field name against your API docs
            JSONObject body = new JSONObject();
            body.put("timestamp", Instant.now().toEpochMilli());
            body.put("order", order);
            String resp = authPost(BASE_URL + "/exchange/v1/derivatives/futures/orders/create", body.toString());
            ...
            System.err.println("placeReduceOnlyOrder: " + e.getMessage());
            return null;
        }
    }
    ...
    // =========================================================================
    // STATE PERSISTENCE — bot_state.json (cooldowns + per-position TP1/trailing state)
    // Save this file as a GitHub Actions artifact between runs, same pattern
    // used previously for last_trade_state.json, otherwise trailing/TP1
    // tracking resets every run.
    // =========================================================================
    private static void loadState() {
        try {
            File f = new File(STATE_FILE);
            if (!f.exists()) { System.out.println("No existing " + STATE_FILE + " — starting fresh."); return; }
            String content = new String(Files.readAllBytes(Paths.get(STATE_FILE)), StandardCharsets.UTF_8);
            JSONObject root = new JSONObject(content);

            JSONObject cooldowns = root.optJSONObject("cooldowns");
            if (cooldowns != null) for (String pair : cooldowns.keySet()) lastTradeTime.put(pair, cooldowns.getLong(pair));

            JSONObject positions = root.optJSONObject("positions");
            if (positions != null) for (String pair : positions.keySet()) positionStates.put(pair, PositionState.fromJson(positions.getJSONObject(pair)));

            System.out.println("Loaded state: " + lastTradeTime.size() + " cooldowns, " + positionStates.size() + " tracked positions.");
        } catch (Exception e) {
            System.err.println("loadState: " + e.getMessage());
        }
    }
    ...
    private static void saveState() {
        try {
            ...
            for (Map.Entry<String, Long> e : lastTradeTime.entrySet()) cooldowns.put(e.getKey(), e.getValue());
            root.put("cooldowns", cooldowns);

            JSONObject positions = new JSONObject();
            for (Map.Entry<String, PositionState> e : positionStates.entrySet()) positions.put(e.getKey(), e.getValue().toJson());
            root.put("positions", positions);

            Files.write(Paths.get(STATE_FILE), root.toString(2).getBytes(StandardCharsets.UTF_8));
            System.out.println("Saved state to " + STATE_FILE);
        } catch (Exception e) {
            System.err.println("saveState: " + e.getMessage());
        }
    }
...
// /**
//  * ═══════════════════════════════════════════════════════════════════════════
//  * CoinDCX Futures Trader — TIMEFRAME-SHIFTED REWRITE (v9)
//  * ═══════════════════════════════════════════════════════════════════════════
//  *
//  * WHAT CHANGED FROM THE PREVIOUS (4H/1H/15M) VERSION:
//  *
//  *   Har cascade level ek "notch" neeche shift kiya gaya hai, taaki chote,
//  *   jyada frequent TP capture ho sakein:
//  *
//  *     OLD                          NEW
//  *     ────────────────────────    ────────────────────────
//  *     4H  = macro trend filter -> 1H  = macro trend filter
//  *     1H  = confirmation        -> 30M = confirmation
//  *     15M = entry timing        -> 5M  = entry timing (pullback + rejection)
//  *
//  *   SL ab ALAG hai in teeno se — user ne specifically 2H Supertrend maanga:
//  *     SL = 2H Supertrend line ± ATR buffer (2H candles), extended past
//  *          recent 2H swing low/high "if practical", hard % cap se protected.
//  *     (2H CoinDCX resolution list me nahi hai — 1H candles ko 2-2 group
//  *      karke aggregate kiya jata hai, bilkul jaise pehle 4H = 1H×4 tha.)
//  *
//  *   TP: same fixed R:R (RR_TARGET) jo pehle tha — chota TP intentionally.
//  *
//  *   RELIABILITY FIXES (the "kabhi kabhi TP/SL populate nahi hota" bug):
//  *     1) findPosition() page size 20 -> 100 (30 open positions tak hone par
//  *        pehle position hi nahi milta tha kabhi kabhi -> TP/SL set hi nahi
//  *        ho pata tha)
//  *     2) getPositionId() ab retry karta hai (pehle sirf 1 attempt tha —
//  *        order abhi-abhi fill hone ke turant baad position exchange pe
//  *        register hone me lag sakta hai)
//  *     3) TPSL_MAX_RETRIES 3->5, TPSL_RETRY_DELAY_MS 2000->3000
//  *     4) SL/TP exchange ko bhejne se pehle sanity-check + tick-distance
//  *        clamp (galat-direction ya bahut-paas SL/TP silently reject ho
//  *        jata tha exchange se)
//  *     5) Safety sweep aur normal entry — dono ab EXACT same 2H-based
//  *        computeSlTp() formula use karte hain
//  *
//  * ═══════════════════════════════════════════════════════════════════════════
//  */
// public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
...
//     // NEW: retry loop specifically for finding the position id right after
//     // an order fills (root cause of "TP/SL kabhi set nahi hota")
//     private static final int  POSITION_ID_MAX_RETRIES = 5;
//     private static final long POSITION_ID_RETRY_DELAY_MS = 1000L;

//     private static final int  TPSL_MAX_RETRIES    = 5;      // was 3
//     private static final long TPSL_RETRY_DELAY_MS = 3000L;  // was 2000
...
//     // ── Entry-timing (5M) parameters ─────────────────────────────────────────
//     // How close (in ATR units, 5m) price must be to 9EMA/21EMA/ST-line to
//     // count as a valid "pullback".
//     private static final double PULLBACK_MAX_ATR = 0.6;

//     // ── SL parameters (now based on 2H Supertrend, per user request) ────────
...
//     private static final int    SWING_LOOKBACK       = 20; // 2H candles, for swing low/high check
...
//     // NEW: minimum distance (in ticks) SL/TP must be from entry, else the
//     // exchange silently rejects create_tpsl — this is one of the reasons
//     // TP/SL "populate" nahi hota tha.
//     private static final int MIN_TICKS_AWAY = 2;

//     // ── Candle fetch counts (SHIFTED cascade) ────────────────────────────────
//     private static final int CANDLE_5M  = 100;  // entry timing (was 15M)
//     private static final int CANDLE_30M = 100;  // confirmation (was 1H)
//     private static final int CANDLE_1H  = 100;  // macro trend   (was 4H)
//     // 2H isn't natively supported by CoinDCX resolution param — derived by
//     // aggregating an extended 1H fetch (same trick the old code used for 4H).
//     private static final int HTF_2H_FETCH_COUNT = 400;
...
//     // the bullish/bearish checklist: Supertrend color, price vs ST line,
//     // EMA9 vs EMA21, price vs both EMAs
//     // =========================================================================
...
//     // =========================================================================
//     // 5M rejection-candle patterns (Hammer / Engulfing / strong candle)
//     // =========================================================================
...
//     // =========================================================================
//     // swing low / swing high helper (2H now), for the SL "if practical" rule
//     // =========================================================================
...
//     // =========================================================================
//     // Unified SL/TP formula — used both at entry time AND by the end-of-scan
//     // safety sweep, so both paths always agree.
//     //   SL = 2H Supertrend line ± ATR buffer (2H ATR), extended past recent
//     //        2H swing low/high "if practical", capped by SL_MAX_PERCENT.
//     //   TP = entry ± RR_TARGET * risk
//     //   tfSl = TFResult built from the 2H (aggregated) candles.
//     // =========================================================================
//     private static double[] computeSlTp(boolean isLong, double entryPrice, TFResult tfSl, double tickSize) {
//         double sl, tp;
//         if (isLong) {
//             double raw = tfSl.stBands[0] - SL_ATR_BUFFER * tfSl.atr;
//             if (raw >= entryPrice) raw = entryPrice - (SL_ATR_BUFFER + 1.5) * tfSl.atr; // ST line stale/invalid — fallback
//             double hardFloor = entryPrice * (1 - SL_MAX_PERCENT / 100.0);

//             double swingLow = findSwingLow(tfSl.lo, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
//             if (swingLow < raw && swingLow > hardFloor) {
//                 raw = swingLow - SWING_EXTRA_BUFFER_ATR * tfSl.atr;
//             }
...
//         } else {
//             double raw = tfSl.stBands[1] + SL_ATR_BUFFER * tfSl.atr;
//             if (raw <= entryPrice) raw = entryPrice + (SL_ATR_BUFFER + 1.5) * tfSl.atr;
//             double hardCeil = entryPrice * (1 + SL_MAX_PERCENT / 100.0);

//             double swingHigh = findSwingHigh(tfSl.hi, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
//             if (swingHigh > raw && swingHigh < hardCeil) {
//                 raw = swingHigh + SWING_EXTRA_BUFFER_ATR * tfSl.atr;
//             }
...
//         // NEW: sanity clamp — exchange silently rejects create_tpsl when
//         // stop/limit price is on the wrong side of entry, or too close to
//         // it (< MIN_TICKS_AWAY ticks). This was one cause of "TP/SL kabhi
//         // set nahi hota".
//         double minGap = tickSize * MIN_TICKS_AWAY;
//         if (isLong) {
//             if (entryPrice - sl < minGap) sl = entryPrice - minGap;
//             if (tp - entryPrice < minGap) tp = entryPrice + minGap;
//         } else {
//             if (sl - entryPrice < minGap) sl = entryPrice + minGap;
//             if (entryPrice - tp < minGap) tp = entryPrice - minGap;
//         }
//         sl = roundToTick(sl, tickSize);
//         tp = roundToTick(tp, tickSize);
...
//                 // ── Fetch candles (SHIFTED cascade + 2H for SL) ───────────────
//                 JSONArray raw5m          = getCandlestickData(pair, "5", CANDLE_5M);
//                 JSONArray raw30m         = getCandlestickData(pair, "30", CANDLE_30M);
//                 JSONArray raw1hExtended  = getCandlestickData(pair, "60", HTF_2H_FETCH_COUNT);
...
//                 // ─────────────────────────────────────────────────────────────
//                 // STEP 1 — 1H macro trend filter
//                 // ─────────────────────────────────────────────────────────────
//                 TFResult tfMacro = analyzeTF(raw1h);
//                 if (!tfMacro.valid) {
//                     System.out.println("  [1H] insufficient data — skip"); continue;
//                 }
//                 System.out.printf("  [1H] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
//                         tfMacro.stGreen ? "GREEN" : "RED", tfMacro.ema9, tfMacro.ema21, tfMacro.price,
//                         tfMacro.bullish ? "BULLISH" : tfMacro.bearish ? "BEARISH" : "NO CLEAR TREND");
//                 if (!tfMacro.bullish && !tfMacro.bearish) {
//                     System.out.println("  1H FAIL — macro trend not clean — skip"); continue;
//                 }

//                 // ─────────────────────────────────────────────────────────────
//                 // STEP 2 — 30M trade confirmation (must match 1H exactly)
//                 // ─────────────────────────────────────────────────────────────
//                 TFResult tfConfirm = analyzeTF(raw30m);
//                 if (!tfConfirm.valid) {
//                     System.out.println("  [30M] insufficient data — skip"); continue;
//                 }
//                 System.out.printf("  [30M] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
//                         tfConfirm.stGreen ? "GREEN" : "RED", tfConfirm.ema9, tfConfirm.ema21, tfConfirm.price,
//                         tfConfirm.bullish ? "BULLISH" : tfConfirm.bearish ? "BEARISH" : "NO CLEAR TREND");

//                 boolean trendUp;
//                 if (tfMacro.bullish && tfConfirm.bullish) {
//                     trendUp = true;
//                 } else if (tfMacro.bearish && tfConfirm.bearish) {
//                     trendUp = false;
//                 } else {
//                     System.out.println("  30M/1H DISAGREE (or 30M unclear) — no trade, wait — skip");
//                     continue;
//                 }
//                 System.out.println("  1H+30M OK — " + (trendUp ? "BULLISH" : "BEARISH") + " confirmed on both");

//                 // ─────────────────────────────────────────────────────────────
//                 // STEP 3 — 5M entry timing
//                 // ─────────────────────────────────────────────────────────────
//                 double[] cl5 = extractCloses(raw5m);
//                 double[] op5 = extractOpens(raw5m);
//                 double[] hi5 = extractHighs(raw5m);
//                 double[] lo5 = extractLows(raw5m);
//                 int n5 = cl5.length;

//                 double ema9_5  = calcEMA(cl5, EMA_FAST);
//                 double ema21_5 = calcEMA(cl5, EMA_MID);
//                 double atr5    = calcATR(hi5, lo5, cl5, ATR_PERIOD);
//                 boolean[] st5Series = calcSupertrend(hi5, lo5, cl5, ST_PERIOD, ST_MULTIPLIER);
//                 boolean stBull5 = st5Series[st5Series.length - 1];
//                 double[] stBands5 = calcSupertrendBands(hi5, lo5, cl5, ST_PERIOD, ST_MULTIPLIER);

//                 // 5M Supertrend + EMA must still agree with the trade direction
//                 boolean tf5Aligned = trendUp
//                         ? (stBull5 && ema9_5 > ema21_5)
//                         : (!stBull5 && ema9_5 < ema21_5);
//                 System.out.printf("  [5M] ST=%s EMA9=%.6f EMA21=%.6f → %s%n",
//                         stBull5 ? "GREEN" : "RED", ema9_5, ema21_5,
//                         tf5Aligned ? "ALIGNED" : "NOT ALIGNED");
//                 if (!tf5Aligned) {
//                     System.out.println("  5M FAIL — not aligned with 1H/30M direction — skip"); continue;
//                 }

//                 // Use the most recently CLOSED 5m candle as the entry candle
//                 if (n5 < 3) { System.out.println("  Not enough 5m candles for entry check — skip"); continue; }
//                 double entryClose = cl5[n5 - 2], entryOpen = op5[n5 - 2];
//                 double entryHigh  = hi5[n5 - 2], entryLow  = lo5[n5 - 2];
//                 double prevClose  = cl5[n5 - 3], prevOpen  = op5[n5 - 3];

//                 // Pullback check: close to 9EMA / 21EMA / ST line
//                 double distEma9  = Math.abs(entryClose - ema9_5);
//                 double distEma21 = Math.abs(entryClose - ema21_5);
//                 double distSt    = trendUp
//                         ? Math.abs(entryClose - stBands5[0])
//                         : Math.abs(entryClose - stBands5[1]);
//                 double maxDist   = PULLBACK_MAX_ATR * atr5;
...
//                 System.out.printf("  [5M-Rejection] %s candle → %s%n",
//                         trendUp ? "Bullish" : "Bearish", rejectionOk ? "CONFIRMED" : "not present");
//                 if (!rejectionOk) {
//                     System.out.println("  5M FAIL — no rejection candle confirmation — skip"); continue;
//                 }
//                 System.out.println("  5M OK — pullback + rejection candle confirmed");
...
//                 // ── SL/TP using the unified 2H-Supertrend formula ─────────────
//                 TFResult tf2h = analyzeTF(raw2h);
//                 if (!tf2h.valid) {
//                     System.out.println("  [2H] insufficient data for SL calc — safety sweep will retry next run");
//                     active.add(pair);
//                     continue;
//                 }
...
//                 // NEW: retry loop for position id (was a single, unretried call)
//                 String posId = getPositionIdWithRetry(pair);
//                 if (posId != null) {
...
//     // =========================================================================
//     // Safety sweep — catches positions whose TP/SL never got set. Recomputes
//     // a fresh 2H TFResult and reuses the SAME computeSlTp() formula as the
//     // main entry path, so sweep-protected positions get identical SL/TP logic.
//     // =========================================================================
...
//                 // CoinDCX convention: positive active_pos = long, negative = short.
...
//                 String posId = pos.optString("id", null);
//                 if (posId != null) {
//                     System.out.printf("  [SWEEP] %s fallback SL=%.6f TP=%.6f (RR=1:%.1f)%n", pair, sl, tp, RR_TARGET);
...
//     // NEW: retry wrapper around getPositionId — a fresh fill sometimes isn't
//     // registered on the exchange yet the instant we ask for it.
//     private static String getPositionIdWithRetry(String pair) {
//         for (int i = 0; i < POSITION_ID_MAX_RETRIES; i++) {
//             String id = getPositionId(pair);
//             if (id != null) return id;
...
//     // NOTE: page size bumped 20 -> 100. With up to MAX_OPEN_POSITIONS (30)
//     // positions live at once, size=20 could miss the pair entirely and
//     // silently skip TP/SL — this was a real root cause of the bug.
//     private static JSONObject findPosition(String pair) throws Exception {
//         JSONObject body = new JSONObject();
//         body.put("timestamp", Instant.now().toEpochMilli());
//         body.put("page", "1");
...
//     // candle. Used for 2H (since CoinDCX API doesn't support resolution=120
//     // natively — derived from the extended 1H fetch).
//     // =========================================================================
//     private static JSONArray aggregateCandles(JSONArray source, int groupSize) {
//         if (source == null || source.length() < groupSize) return null;
//         int n = source.length();
...
// /**
//  * ═══════════════════════════════════════════════════════════════════════════
//  * CoinDCX Futures Trader — REQUIREMENT-BASED REWRITE
//  * ═══════════════════════════════════════════════════════════════════════════
//  *
//  * WHAT CHANGED FROM v19:
//  *
//  *   REMOVED entirely (old long/short decision logic):
//  *     - BTC direction gate (G0)
//  *     - 5-timeframe (5m/15m/30m/1H/2H) EMA+Supertrend confluence scoring
//  *     - ADX, MACD, RSI soft filters
//  *     - Choppiness Index (G-CHOP)
//  *     - Support/Resistance proximity gate (G-SR)
//  *     - Funding rate gate (G-FUND)
//  *     - Volume confirmation gate (G-VOL)
//  *     - Multi-candle / RSI-divergence gates
//  *     - Spread gate (G-SPREAD)
//  *
//  *   NEW direction/entry logic (from the strategy document), and ONLY this:
//  *     1) 4H timeframe = macro trend filter
//  *          Supertrend GREEN/RED + price vs ST line + EMA9 vs EMA21 + price vs EMAs
//  *     2) 1H timeframe = trade confirmation — must match 4H direction exactly.
//  *          If 4H and 1H disagree → NO TRADE (wait), per the document's
//  *          "Case 1 / Case 2" rule.
//  *     3) 15M timeframe = entry timing
//  *          - 15M Supertrend + EMA9/EMA21 must still agree with the direction
//  *          - Price must be in a pullback (near 9/21 EMA or near the ST line)
//  *          - A rejection candle (Hammer / Engulfing / strong momentum candle)
//  *            must close in the trade direction
//  *     4) Stop Loss: 1H Supertrend line ± ATR buffer (0.2–0.5 ATR, using 0.35),
//  *        extended to just beyond the recent 1H swing low/high "if practical",
//  *        always capped by a hard % cap for leverage safety.
//  *     5) Take Profit: fixed R:R of 1.8 (midpoint of the document's 1.5–2.0 range).
//  *
//  *   KEPT AS-IS (execution reliability infra — unrelated to signal logic):
...
//  *     - ensureTpSlForOpenPositions() — end-of-scan safety sweep for any
//  *       position that filled late and missed TP/SL, now using the SAME
//  *       1H-Supertrend-based SL/TP formula as the main entry path (previously
//  *       the sweep used a different ad-hoc fallback — now unified)
//  *     - Cooldown, MAX_OPEN_POSITIONS, position/quantity/HMAC/HTTP plumbing
//  *
//  * ═══════════════════════════════════════════════════════════════════════════
//  */
// public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
...
//     // ── NEW: requirement-based entry parameters ──────────────────────────────
...
//     // ── NEW: requirement-based SL parameters ─────────────────────────────────
//     // Document says: "slightly below the 1H Supertrend line... 0.2-0.5 ATR buffer"
//     private static final double SL_ATR_BUFFER   = 0.35;
//     private static final double SL_MAX_PERCENT  = 4.5;   // hard cap — leverage safety net
//     private static final int    SWING_LOOKBACK       = 20; // 1H candles, for swing low/high check
//     private static final int    SWING_EXCLUDE_RECENT = 2;
//     private static final double SWING_EXTRA_BUFFER_ATR = 0.15;

//     // ── NEW: requirement-based TP parameter ──────────────────────────────────
//     // Document says R:R 1.5:1 to 2:1 — using the midpoint as a fixed target.
//     private static final double RR_TARGET = 0.1;
...
//     // 4H isn't natively supported by CoinDCX resolution param — derived by
//     // aggregating an extended 1H fetch (same trick as before).
...
//     // =========================================================================
//     // NEW: TFResult — holds everything needed to judge one timeframe against
//     // the document's bullish/bearish checklist:
...
//         // Document's exact checklist per TF:
//         r.bullish = r.stGreen && priceAboveSt && (r.ema9 > r.ema21) && priceAboveEmas;
//         r.bearish = (!r.stGreen) && priceBelowSt && (r.ema9 < r.ema21) && priceBelowEmas;
//         return r;
//     }
...
//     // =========================================================================
//     // NEW: 15M rejection-candle patterns (Hammer / Engulfing / strong candle),
...
//     // =========================================================================
//     // NEW: swing low / swing high helper (1H), for the SL "if practical" rule
//     // =========================================================================
//     private static double findSwingLow(double[] lo, int lookback, int excludeRecent) {
//         int n = lo.length;
//         int start = Math.max(0, n - lookback - excludeRecent);
...
//     // =========================================================================
//     // NEW: unified SL/TP formula — used both at entry time AND by the
//     // end-of-scan safety sweep, so both paths always agree.
//     //   SL = 1H Supertrend line ± ATR buffer, extended past recent swing
//     //        low/high "if practical", capped by SL_MAX_PERCENT.
//     //   TP = entry ± RR_TARGET * risk
//     // =========================================================================
//     private static double[] computeSlTp(boolean isLong, double entryPrice, TFResult tf1h, double tickSize) {
//         double sl, tp;
//         if (isLong) {
//             double raw = tf1h.stBands[0] - SL_ATR_BUFFER * tf1h.atr;
//             if (raw >= entryPrice) raw = entryPrice - (SL_ATR_BUFFER + 1.5) * tf1h.atr; // ST line stale/invalid — fallback
//             double hardFloor = entryPrice * (1 - SL_MAX_PERCENT / 100.0);

//             // "if practical": push SL just below the recent 1H swing low, but
//             // never past the hard % cap.
//             double swingLow = findSwingLow(tf1h.lo, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
//             if (swingLow < raw && swingLow > hardFloor) {
//                 raw = swingLow - SWING_EXTRA_BUFFER_ATR * tf1h.atr;
//             }
...
//             tp = entryPrice + RR_TARGET * risk;
//         } else {
//             double raw = tf1h.stBands[1] + SL_ATR_BUFFER * tf1h.atr;
//             if (raw <= entryPrice) raw = entryPrice + (SL_ATR_BUFFER + 1.5) * tf1h.atr;
//             double hardCeil = entryPrice * (1 + SL_MAX_PERCENT / 100.0);

//             double swingHigh = findSwingHigh(tf1h.hi, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
//             if (swingHigh > raw && swingHigh < hardCeil) {
//                 raw = swingHigh + SWING_EXTRA_BUFFER_ATR * tf1h.atr;
//             }
...
//                 // ─────────────────────────────────────────────────────────────
//                 // STEP 2 — 1H trade confirmation (must match 4H exactly)
//                 // ─────────────────────────────────────────────────────────────
...
//                 boolean trendUp;
//                 if (tf4h.bullish && tf1h.bullish) {
//                     trendUp = true;
//                 } else if (tf4h.bearish && tf1h.bearish) {
//                     trendUp = false;
//                 } else {
//                     System.out.println("  1H/4H DISAGREE (or 1H unclear) — per doc rule: no trade, wait — skip");
//                     continue;
//                 }
//                 System.out.println("  4H+1H OK — " + (trendUp ? "BULLISH" : "BEARISH") + " confirmed on both");
...
//                 if (!tf15Aligned) {
//                     System.out.println("  15M FAIL — not aligned with 4H/1H direction — skip"); continue;
//                 }
...
//                 // ── SL/TP using the unified 1H-ST formula ─────────────────────
...
//                     setTpSlWithRetry(posId, tpPrice, slPrice, pair);
//                 } else {
//                     System.out.println("  Position ID not found — TP/SL will be handled by end-of-scan safety sweep");
//                 }
...
//     // entry confirmation timed out mid-loop. Now recomputes 1H TFResult fresh
//     // and reuses the SAME computeSlTp() formula as the main entry path, so
//     // sweep-protected positions get identical SL/TP logic to normal entries.
//     // =========================================================================
//     private static void ensureTpSlForOpenPositions() {
//         try {
//             Set<String> stillOpen = getActivePositions();
...
//     // =========================================================================
//     // TP/SL retry-and-confirm setter — unchanged from v19
//     // =========================================================================
//     private static boolean setTpSlWithRetry(String posId, double tp, double sl, String pair) {
//         for (int attempt = 1; attempt <= TPSL_MAX_RETRIES; attempt++) {
//             setTpSl(posId, tp, sl, pair);
...
//     // =========================================================================
//     // AGGREGATE CANDLES: merges N consecutive smaller candles into 1 bigger
//     // candle. Used for 4H (since CoinDCX API doesn't support resolution=240
//     // natively — derived from the extended 1H fetch).
//     // =========================================================================
//     private static JSONArray aggregateCandles(JSONArray source, int groupSize) {
//         if (source == null || source.length() < groupSize) return null;
...
// /**
//  * ═══════════════════════════════════════════════════════════════════════════
//  * CoinDCX Futures Trader — v19 (post loss-analysis + robustness patch)
//  * ═══════════════════════════════════════════════════════════════════════════
//  *
//  * CHANGES FROM v14-FIXED → v19:
//  *
//  *   FIX #7  — Volume confirmation (G-VOL)
//  *     Entry candle (15m) volume must be >= MIN_VOLUME_RATIO (1.1x) of the
//  *     average of the previous VOLUME_LOOKBACK (20) candles. If the candle
//  *     payload has no "volume" field, the gate is skipped gracefully (never
//  *     blocks a trade due to missing data).
//  *
//  *   FIX #8  — 4H / Daily higher-timeframe structure (G-HTF)
//  *     4H EMA9/21 + Supertrend is computed from an extended 1H candle fetch
//  *     (aggregated in-process, since CoinDCX doesn't support resolution=240
//  *     natively). Only a FULL disagreement (both EMA and Supertrend opposite
//  *     of trade direction) blocks the trade — partial disagreement is
//  *     allowed, keeping this gate lenient. Daily bias is logged only
//  *     (informational, non-blocking) since daily candle counts are borderline
//  *     for reliable EMA21.
//  *
//  *   FIX #9  — Support/Resistance proximity check (G-SR)
//  *     Before entering, the nearest prior swing high (for longs) / swing low
//  *     (for shorts) over the last SR_LOOKBACK (50) 30m candles is located.
//  *     If price is closer than SR_MIN_ATR_DIST (0.3x ATR) to that level, the
//  *     trade is skipped — avoids buying directly under resistance / selling
//  *     directly above support.
//  *
//  *   FIX #10 — Funding rate gate (G-FUND)
//  *     Reads "funding_rate" from the cached instrument data (field name may
//  *     need adjusting to match the live CoinDCX response — see comment on
//  *     getFundingRate). Blocks longs when funding is extremely positive
//  *     (crowded longs) and shorts when extremely negative (crowded shorts).
//  *     Gate is skipped (non-blocking) if the field isn't available.
//  *     NOTE: Open-Interest trend tracking needs persisted state across runs
//  *     (similar to the last_trade_state.json pattern from v17) — a stub/TODO
//  *     is included but left non-blocking for now.
//  *
//  *   FIX #11 — Choppiness Index (G-CHOP)
//  *     Complements ADX: CI = 100*log10(sum(TR,14)/(max(high,14)-min(low,14)))
//  *     / log10(14) on 30m. CI above CHOPPINESS_MAX (61.8) means the market is
//  *     range-bound even if ADX briefly ticks over threshold — skip.
//  *
//  *   FIX #12 — Multi-candle confirmation (extends 15m momentum check)
//  *     Previously a single 15m candle decided momentum. Now the candle
//  *     before it must also close in the same direction (no strength
//  *     requirement on the second candle — just directional agreement).
//  *
//  *   FIX #13 — RSI/price divergence detection (G-DIV)
//  *     On 30m: if trade is a long and price makes a higher high than a
//  *     recent swing while RSI makes a lower high → bearish divergence → skip.
//  *     Symmetric check for shorts (bullish divergence on lower low).
//  *
//  *   FIX #14 — Spread/liquidity check (G-SPREAD)
//  *     Immediately before placing the order, fetches best bid/ask and skips
//  *     if the spread exceeds SPREAD_MAX_PCT (0.15%). Gate is skipped
//  *     (non-blocking) if the orderbook endpoint/parsing fails — verify the
//  *     response shape against the live CoinDCX orderbook endpoint.
//  *
//  *   FIX #15 — TP/SL reliability fix (the "missing TP/SL" bug)
//  *     Root cause: limit orders can take longer to fill than the old 10s
//  *     entry-confirmation window, and setTpSl was a single fire-and-forget
//  *     call with no verification. Fixes:
//  *       a) MAX_ENTRY_PRICE_CHECKS raised 10 → 20 (more time to confirm fill).
//  *       b) setTpSlWithRetry() now calls the API up to TPSL_MAX_RETRIES (3)
//  *          times, re-fetching the position after each attempt to CONFIRM
//  *          take_profit_trigger/stop_loss_trigger are actually non-zero.
//  *       c) ensureTpSlForOpenPositions() runs at the END of every scan as a
//  *          safety sweep: any open position with avg_price>0 but missing
//  *          TP/SL gets a fallback SL/TP computed fresh from live 30m ATR and
//  *          applied via the same retrying setter. This catches positions
//  *          that filled *after* the main loop's confirmation window expired.
//  *
//  * ═══════════════════════════════════════════════════════════════════════════
//  */
// public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
...
//     // ── FIX #15a: more time allowed for limit-order fill confirmation ───────
//     private static final int    MAX_ENTRY_PRICE_CHECKS = 20;
//     private static final int    ENTRY_CHECK_DELAY_MS   = 1000;

//     // ── FIX #15b: TP/SL retry-and-confirm parameters ─────────────────────────
//     private static final int  TPSL_MAX_RETRIES   = 3;
//     private static final long TPSL_RETRY_DELAY_MS = 2000L;
...
//     // ── FIX #8: extended 1H fetch used to derive 2H / 4H / Daily candles ────
//     // (700 hourly candles ≈ 29 days — enough for 4H EMA21 comfortably, and
//     //  borderline-but-workable for Daily EMA21; Daily is informational-only
//     //  below precisely because of this borderline sample size.)
//     private static final int HTF_1H_FETCH_COUNT = 700;

//     // ── FIX #3: Limit order slippage buffer ──────────────────────────────────
//     private static final double LIMIT_ORDER_BUFFER_PCT = 0.001; // 0.1%

//     // ── Daily P&L protection ──────────────────────────────────────────────────
//     private static final double DAILY_PROFIT_TARGET  = 40000.0;
//     private static final double DAILY_DRAWDOWN_LIMIT = 25000.0;
//     private static final double DAILY_LOSS_LIMIT     = 30000.0;

//     // ── FIX #7: Volume confirmation ───────────────────────────────────────────
//     private static final double MIN_VOLUME_RATIO = 1.1;   // entry candle vs avg
//     private static final int    VOLUME_LOOKBACK  = 20;

//     // ── FIX #11: Choppiness Index ─────────────────────────────────────────────
//     private static final int    CHOPPINESS_PERIOD = 14;
//     private static final double CHOPPINESS_MAX    = 61.8;

//     // ── FIX #9: Support/Resistance proximity ─────────────────────────────────
//     private static final int    SR_LOOKBACK      = 50;
//     private static final int    SR_EXCLUDE_RECENT = 3;
//     private static final double SR_MIN_ATR_DIST  = 0.3;

//     // ── FIX #10: Funding rate extreme threshold (0.1%) ───────────────────────
//     private static final double FUNDING_RATE_EXTREME = 0.001;

//     // ── FIX #14: Spread/liquidity check ──────────────────────────────────────
//     private static final double SPREAD_MAX_PCT = 0.15; // percent

//     // ── FIX #13: Divergence lookback window ──────────────────────────────────
//     private static final int DIVERGENCE_LOOKBACK       = 20;
//     private static final int DIVERGENCE_EXCLUDE_RECENT = 3;
...
//     // =========================================================================
//     // Coin list — top-40-ish liquid USDT pairs
//     // =========================================================================
//     private static final String[] COIN_SYMBOLS = {
//         "ETH", "SOL", "ZEC", "XRP", "DOGE", "BNB", "TAO", "1000PEPE", "ADA", "SUI",
//         "BCH", "LINK", "AVAX", "FIL", "OP", "NEAR", "TRX", "TRUMP", "ARB", "WLD",
...
//     // =========================================================================
//     // FIX #2: BTC direction gate (G0)
//     // =========================================================================
//     private static Boolean getBtcDirection() {
//         try {
//             JSONArray btc1h = getCandlestickData("B-BTC_USDT", "60", CANDLE_1H);
//             if (btc1h == null || btc1h.length() < EMA_MID + 5) return null;
//             double[] cl = extractCloses(btc1h);
//             double[] hi = extractHighs(btc1h);
//             double[] lo = extractLows(btc1h);
//             boolean emaBull = calcEMA(cl, EMA_FAST) > calcEMA(cl, EMA_MID);
//             boolean[] st = calcSupertrend(hi, lo, cl, ST_PERIOD, ST_MULTIPLIER);
//             boolean stBull = st[st.length - 1];
//             if (emaBull == stBull) return emaBull;
//             return stBull;
//         } catch (Exception e) {
//             System.err.println("getBtcDirection: " + e.getMessage());
//             return null;
//         }
//     }
...
//             System.out.println("MAX_OPEN_POSITIONS (" + MAX_OPEN_POSITIONS +
//                     ") already reached (" + active.size() + " open) — skipping scan entirely.");
//             // ── FIX #15c: still run the safety sweep even if scan is skipped ──
//             ensureTpSlForOpenPositions();
//             return;
//         }

//         Boolean btcBullish = getBtcDirection();
//         System.out.println("BTC 1H direction: " +
//                 (btcBullish == null ? "UNKNOWN (gate skipped)" : (btcBullish ? "BULLISH" : "BEARISH")));
...
//                 // ── FIX #8: single extended 1H fetch feeds 1H/2H/4H/Daily ─────
//                 JSONArray raw1hExtended = getCandlestickData(pair, "60", HTF_1H_FETCH_COUNT);
//                 JSONArray raw1h    = lastN(raw1hExtended, CANDLE_1H);
//                 JSONArray raw2h    = aggregateCandles(raw1hExtended, 2);
//                 JSONArray raw4h    = aggregateCandles(raw1hExtended, 4);
//                 JSONArray rawDaily = aggregateCandles(raw1hExtended, 24);
...
//                 System.out.printf("  Price=%.6f  ATR-30m=%.6f  ATR-15m=%.6f  ATR-1H=%.6f  Tick=%.8f%n",
//                         lastClose, atr30m, atr15m, atr1h, tickSize);
...
//                 // ─────────────────────────────────────────────────────────────
//                 // MTF EMA9/21 + SUPERTREND CONFLUENCE FILTER (unchanged 5-TF core)
//                 // ─────────────────────────────────────────────────────────────
//                 System.out.println("  ── MTF EMA9/21 + Supertrend Check ────────");
...
//                 if (btcBullish != null && btcBullish != trendUp) {
//                     System.out.printf("  [G0-BTC] BTC=%s vs Candidate=%s → conflict — skip%n",
//                             btcBullish ? "BULL" : "BEAR", trendUp ? "BULL" : "BEAR");
//                     continue;
//                 } else if (btcBullish != null) {
//                     System.out.println("  G0 OK — BTC direction agrees");
//                 }

//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #8: G-HTF — 4H (blocking on full disagreement only)
//                 //          Daily (informational only, non-blocking)
//                 // ─────────────────────────────────────────────────────────────
//                 MtfSignal htf4h;
//                 if (raw4h != null && raw4h.length() >= EMA_MID + 5) {
//                     double[] cl4h = extractCloses(raw4h);
//                     double[] hi4h = extractHighs(raw4h);
//                     double[] lo4h = extractLows(raw4h);
//                     boolean emaBull4h = calcEMA(cl4h, EMA_FAST) > calcEMA(cl4h, EMA_MID);
//                     boolean[] st4h = calcSupertrend(hi4h, lo4h, cl4h, ST_PERIOD, ST_MULTIPLIER);
//                     htf4h = new MtfSignal("4H", emaBull4h, st4h[st4h.length-1], true, true);
//                 } else {
//                     htf4h = new MtfSignal("4H", false, false, false, false);
//                 }
//                 if (htf4h.emaValid && htf4h.stValid) {
//                     boolean htf4hFullyOpposed = (htf4h.emaBull != trendUp) && (htf4h.stBull != trendUp);
//                     System.out.printf("  [G-HTF-4H] EMA=%s ST=%s vs Candidate=%s → %s%n",
//                             htf4h.emaBull ? "BULL" : "BEAR", htf4h.stBull ? "BULL" : "BEAR",
//                             trendUp ? "BULL" : "BEAR",
//                             htf4hFullyOpposed ? "FULL CONFLICT — skip" : "OK");
//                     if (htf4hFullyOpposed) {
//                         System.out.println("  G-HTF-4H FAIL — 4H fully opposed — skip");
//                         continue;
//                     }
//                 } else {
//                     System.out.println("  [G-HTF-4H] insufficient 4H data — gate skipped");
//                 }

//                 if (rawDaily != null && rawDaily.length() >= EMA_MID + 5) {
//                     double[] clD = extractCloses(rawDaily);
//                     boolean emaBullD = calcEMA(clD, EMA_FAST) > calcEMA(clD, EMA_MID);
//                     System.out.printf("  [INFO-Daily] EMA9/21 bias = %s (informational only, non-blocking)%n",
//                             emaBullD ? "BULL" : "BEAR");
//                 } else {
//                     System.out.println("  [INFO-Daily] insufficient daily data — skipped");
//                 }
...
//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #11: G-CHOP — Choppiness Index (complements ADX)
//                 // ─────────────────────────────────────────────────────────────
//                 double chop = calcChoppiness(hi30, lo30, cl30, CHOPPINESS_PERIOD);
//                 System.out.printf("  [G-CHOP] Choppiness=%.2f (max=%.1f) → %s%n",
//                         chop, CHOPPINESS_MAX, chop <= CHOPPINESS_MAX ? "PASS" : "FAIL — choppy");
//                 if (chop > CHOPPINESS_MAX) {
//                     System.out.println("  CHOP FAIL — market too choppy/ranging — skip"); continue;
//                 }

//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #13: G-DIV — RSI/price divergence check on 30m
//                 // ─────────────────────────────────────────────────────────────
//                 if (trendUp) {
//                     boolean bearDiv = checkBearishDivergence(cl30, DIVERGENCE_LOOKBACK, DIVERGENCE_EXCLUDE_RECENT);
//                     System.out.printf("  [G-DIV] Bearish divergence check → %s%n",
//                             bearDiv ? "DETECTED — skip" : "none");
//                     if (bearDiv) { System.out.println("  DIV FAIL — bearish divergence against long — skip"); continue; }
//                 } else {
//                     boolean bullDiv = checkBullishDivergence(cl30, DIVERGENCE_LOOKBACK, DIVERGENCE_EXCLUDE_RECENT);
//                     System.out.printf("  [G-DIV] Bullish divergence check → %s%n",
//                             bullDiv ? "DETECTED — skip" : "none");
//                     if (bullDiv) { System.out.println("  DIV FAIL — bullish divergence against short — skip"); continue; }
//                 }
...
//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #9: G-SR — Support/Resistance proximity check
//                 // ─────────────────────────────────────────────────────────────
//                 if (trendUp) {
//                     double resistance = findRecentResistance(hi30, lastClose, SR_LOOKBACK, SR_EXCLUDE_RECENT);
//                     if (resistance != Double.POSITIVE_INFINITY) {
//                         double dist   = resistance - lastClose;
//                         double minDist = SR_MIN_ATR_DIST * atr30m;
//                         System.out.printf("  [G-SR] Nearest resistance=%.6f Dist=%.6f MinReq=%.6f → %s%n",
//                                 resistance, dist, minDist, dist >= minDist ? "PASS" : "FAIL — too close");
//                         if (dist < minDist) {
//                             System.out.println("  SR FAIL — price too close to resistance — skip"); continue;
//                         }
//                     } else {
//                         System.out.println("  [G-SR] No nearby resistance found — PASS");
//                     }
//                 } else {
//                     double support = findRecentSupport(lo30, lastClose, SR_LOOKBACK, SR_EXCLUDE_RECENT);
//                     if (support != Double.NEGATIVE_INFINITY) {
//                         double dist   = lastClose - support;
//                         double minDist = SR_MIN_ATR_DIST * atr30m;
//                         System.out.printf("  [G-SR] Nearest support=%.6f Dist=%.6f MinReq=%.6f → %s%n",
//                                 support, dist, minDist, dist >= minDist ? "PASS" : "FAIL — too close");
//                         if (dist < minDist) {
//                             System.out.println("  SR FAIL — price too close to support — skip"); continue;
//                         }
//                     } else {
//                         System.out.println("  [G-SR] No nearby support found — PASS");
//                     }
//                 }
...
//                 // FIX #12: multi-candle confirmation added below
//                 // ─────────────────────────────────────────────────────────────
...
//                 // FIX #12: second-most-recent closed 15m candle must also agree
//                 // directionally (no strength requirement — just direction).
//                 boolean prevPrevBull15m = cl15.length >= 3 && cl15[cl15.length - 3] > op15[op15.length - 3];
//                 boolean prevPrevBear15m = cl15.length >= 3 && cl15[cl15.length - 3] < op15[op15.length - 3];

//                 boolean entry15mOk;
//                 String  entry15mReason;
//                 if (trendUp) {
//                     boolean multiCandleOk = prevPrevBull15m;
//                     entry15mOk     = entry15mBull && strongBody && closeNearHigh && multiCandleOk;
//                     entry15mReason = String.format("Bull=%s Body=%.0f%% ClosePos=%.0f%% PrevCandleBull=%s",
//                             entry15mBull, bodyRatio * 100, closePosition * 100, multiCandleOk);
//                 } else {
//                     boolean multiCandleOk = prevPrevBear15m;
//                     entry15mOk     = entry15mBear && strongBody && closeNearLow && multiCandleOk;
//                     entry15mReason = String.format("Bear=%s Body=%.0f%% ClosePos=%.0f%% PrevCandleBear=%s",
//                             entry15mBear, bodyRatio * 100, closePosition * 100, multiCandleOk);
//                 }
//                 System.out.printf("  [15m-MOMENTUM] %s → %s%n", entry15mReason,
//                         entry15mOk ? "PASS" : "FAIL — weak/unconfirmed candle");
//                 if (!entry15mOk) {
//                     System.out.println("  15m FAIL — momentum candle insufficient — skip"); continue;
//                 }
//                 System.out.println("  15m OK — strong momentum candle with multi-candle confirmation");

//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #7: G-VOL — Volume confirmation
//                 // ─────────────────────────────────────────────────────────────
//                 boolean volOk;
//                 try {
//                     double[] vol15 = extractVolumes(raw15m);
//                     volOk = checkVolumeConfirmation(vol15, VOLUME_LOOKBACK, MIN_VOLUME_RATIO);
//                     System.out.printf("  [G-VOL] entry candle vs %d-avg (min ratio=%.1fx) → %s%n",
//                             VOLUME_LOOKBACK, MIN_VOLUME_RATIO, volOk ? "PASS" : "FAIL — weak volume");
//                 } catch (Exception ve) {
//                     System.out.println("  [G-VOL] volume data unavailable — gate skipped");
//                     volOk = true;
//                 }
//                 if (!volOk) {
//                     System.out.println("  VOL-CONFIRM FAIL — skip"); continue;
//                 }

//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #10: G-FUND — Funding rate gate
//                 // ─────────────────────────────────────────────────────────────
//                 Double fundingRate = getFundingRate(pair);
//                 if (fundingRate != null && !Double.isNaN(fundingRate)) {
//                     boolean fundingBlocked = (trendUp && fundingRate > FUNDING_RATE_EXTREME)
//                                           || (trendDown && fundingRate < -FUNDING_RATE_EXTREME);
//                     System.out.printf("  [G-FUND] FundingRate=%.4f%% (extreme=±%.2f%%) → %s%n",
//                             fundingRate * 100, FUNDING_RATE_EXTREME * 100,
//                             fundingBlocked ? "FAIL — crowded positioning" : "OK");
//                     if (fundingBlocked) {
//                         System.out.println("  FUNDING FAIL — crowded positioning — skip"); continue;
//                     }
//                 } else {
//                     System.out.println("  [G-FUND] funding rate unavailable — gate skipped");
//                 }
//                 // TODO (OI trend): needs persisted open-interest snapshots across
//                 // runs (same pattern as last_trade_state.json from v17) to compare
//                 // OI now vs OI at last scan. Left as non-blocking for this version.
...
//                 // ─────────────────────────────────────────────────────────────
//                 // FIX #14: G-SPREAD — spread/liquidity check right before order
//                 // ─────────────────────────────────────────────────────────────
//                 Double spreadPct = getOrderBookSpreadPercent(pair);
//                 if (spreadPct != null) {
//                     boolean spreadOk = spreadPct <= SPREAD_MAX_PCT;
//                     System.out.printf("  [G-SPREAD] Spread=%.4f%% (max=%.2f%%) → %s%n",
//                             spreadPct, SPREAD_MAX_PCT, spreadOk ? "OK" : "FAIL — too wide");
//                     if (!spreadOk) {
//                         System.out.println("  SPREAD FAIL — orderbook too wide — skip"); continue;
//                     }
//                 } else {
//                     System.out.println("  [G-SPREAD] orderbook unavailable — gate skipped");
//                 }
...
//                         slAtr    = atr1h;
//                     } else {
//                         stBands  = new double[]{entry - 2.4 * atr1h, entry + 2.4 * atr1h};
...
//                 System.out.printf("  SL source: %s ST bands (ATR used=%.6f) | Lower=%.6f Upper=%.6f%n",
//                         slSource, slAtr, stLower, stUpper);

//                 double slPrice, tpPrice;
//                 if ("buy".equalsIgnoreCase(side)) {
//                     double rawSL = stLower - (ST_SL_BUFFER * slAtr);
...
//                 String posId = getPositionId(pair);
//                 if (posId != null) {
//                     // ── FIX #15b: retrying + confirming setter instead of fire-and-forget ──
//                     setTpSlWithRetry(posId, tpPrice, slPrice, pair);
//                 } else {
//                     System.out.println("  Position ID not found — TP/SL will be handled by end-of-scan safety sweep");
//                 }
...
//         // ── FIX #15c: end-of-scan safety sweep for any missed TP/SL ─────────
//         ensureTpSlForOpenPositions();
//     }

//     // =========================================================================
//     // FIX #15c: safety sweep — catches positions whose TP/SL never got set
//     // because entry confirmation timed out mid-loop, or setTpSl failed
//     // silently for any other reason.
//     // =========================================================================
//     private static void ensureTpSlForOpenPositions() {
//         try {
//             Set<String> stillOpen = getActivePositions();
...
//                 System.out.println("  [SWEEP] " + pair + " missing TP/SL — computing fallback protection...");
//                 JSONArray raw30 = getCandlestickData(pair, "30", CANDLE_30M);
//                 if (raw30 == null || raw30.length() < 30) {
//                     System.out.println("  [SWEEP] insufficient data for " + pair + " — could not protect, will retry next run");
//                     continue;
//                 }
//                 double[] hi30 = extractHighs(raw30), lo30 = extractLows(raw30), cl30 = extractCloses(raw30);
//                 double atr = calcATR(hi30, lo30, cl30, ATR_PERIOD);
...
//                 double fallbackRR = 1.5;
//                 double sl, tp;
//                 if (isLong) {
//                     sl = avgPrice - SL_MIN_ATR * atr;
//                     double hardFloor = avgPrice * (1 - SL_MAX_PERCENT / 100.0);
//                     sl = Math.max(sl, hardFloor);
//                     tp = avgPrice + fallbackRR * (avgPrice - sl);
//                 } else {
//                     sl = avgPrice + SL_MIN_ATR * atr;
//                     double hardCeil = avgPrice * (1 + SL_MAX_PERCENT / 100.0);
//                     sl = Math.min(sl, hardCeil);
//                     tp = avgPrice - fallbackRR * (sl - avgPrice);
//                 }
...
//                 String posId = pos.optString("id", null);
//                 if (posId != null) {
//                     System.out.printf("  [SWEEP] %s fallback SL=%.6f TP=%.6f (RR=1:%.1f)%n", pair, sl, tp, fallbackRR);
//                     setTpSlWithRetry(posId, tp, sl, pair);
//                 } else {
//                     System.out.println("  [SWEEP] " + pair + " — position ID missing, cannot set TP/SL");
//                 }
...
//     // =========================================================================
//     // FIX #15b: retrying + confirming TP/SL setter
//     // =========================================================================
//     private static boolean setTpSlWithRetry(String posId, double tp, double sl, String pair) {
//         for (int attempt = 1; attempt <= TPSL_MAX_RETRIES; attempt++) {
//             setTpSl(posId, tp, sl, pair);
...
//     // =========================================================================
//     // FIX #7: Volume helpers
//     // =========================================================================
//     private static double[] extractVolumes(JSONArray a) {
//         double[] o = new double[a.length()];
//         for (int i = 0; i < a.length(); i++) o[i] = a.getJSONObject(i).optDouble("volume", -1);
//         return o;
//     }

//     private static boolean checkVolumeConfirmation(double[] vol, int lookback, double minRatio) {
//         int n = vol.length;
//         int entryIdx = n - 2; // matches the 15m-momentum candle index used elsewhere
//         if (entryIdx - lookback < 0) return true; // not enough history — don't block
//         double sum = 0;
//         for (int i = entryIdx - lookback; i < entryIdx; i++) sum += vol[i];
//         double avg = sum / lookback;
//         if (avg <= 0) return true; // volume field missing/zero — gate skipped, not blocked
//         return vol[entryIdx] >= minRatio * avg;
//     }

//     // =========================================================================
//     // FIX #11: Choppiness Index
//     // =========================================================================
//     private static double calcChoppiness(double[] hi, double[] lo, double[] cl, int period) {
//         int n = hi.length;
//         if (n < period + 1) return 50; // neutral — don't block on insufficient data
//         double sumTR = 0;
//         double maxHigh = Double.NEGATIVE_INFINITY, minLow = Double.POSITIVE_INFINITY;
//         for (int i = n - period; i < n; i++) {
//             double tr = Math.max(hi[i] - lo[i],
//                     Math.max(Math.abs(hi[i] - cl[i-1]), Math.abs(lo[i] - cl[i-1])));
//             sumTR += tr;
//             maxHigh = Math.max(maxHigh, hi[i]);
//             minLow  = Math.min(minLow, lo[i]);
//         }
...
//     // =========================================================================
//     // FIX #9: Support/Resistance helpers
//     // =========================================================================
//     private static double findRecentResistance(double[] hi, double lastClose, int lookback, int excludeRecent) {
//         int n = hi.length;
//         int start = Math.max(0, n - lookback - excludeRecent);
//         int end   = Math.max(start, n - excludeRecent);
//         double resistance = Double.POSITIVE_INFINITY;
//         for (int i = start; i < end; i++) {
//             if (hi[i] > lastClose && hi[i] < resistance) resistance = hi[i];
//         }
//         return resistance;
//     }

//     private static double findRecentSupport(double[] lo, double lastClose, int lookback, int excludeRecent) {
//         int n = lo.length;
//         int start = Math.max(0, n - lookback - excludeRecent);
//         int end   = Math.max(start, n - excludeRecent);
//         double support = Double.NEGATIVE_INFINITY;
//         for (int i = start; i < end; i++) {
//             if (lo[i] < lastClose && lo[i] > support) support = lo[i];
//         }
//         return support;
//     }

//     // =========================================================================
//     // FIX #13: Divergence detection helpers (simple two-swing-point check)
//     // =========================================================================
//     private static boolean checkBearishDivergence(double[] cl, int lookback, int excludeRecent) {
//         int n = cl.length;
//         if (n < lookback + excludeRecent + RSI_PERIOD + 6) return false;
//         int recentIdx = n - excludeRecent - 1;
//         int priorStart = Math.max(0, n - lookback - excludeRecent);
//         int priorEnd = recentIdx - 5;
//         if (priorEnd <= priorStart) return false;
//         int priorIdx = priorStart;
//         for (int i = priorStart; i <= priorEnd; i++) if (cl[i] > cl[priorIdx]) priorIdx = i;
//         if (cl[recentIdx] <= cl[priorIdx]) return false; // not a higher high, no divergence signal
//         double rsiRecent = calcRSI(Arrays.copyOfRange(cl, 0, recentIdx + 1), RSI_PERIOD);
//         double rsiPrior  = calcRSI(Arrays.copyOfRange(cl, 0, priorIdx + 1), RSI_PERIOD);
//         return rsiRecent < rsiPrior;
//     }

//     private static boolean checkBullishDivergence(double[] cl, int lookback, int excludeRecent) {
//         int n = cl.length;
//         if (n < lookback + excludeRecent + RSI_PERIOD + 6) return false;
//         int recentIdx = n - excludeRecent - 1;
//         int priorStart = Math.max(0, n - lookback - excludeRecent);
...
//         int priorIdx = priorStart;
//         for (int i = priorStart; i <= priorEnd; i++) if (cl[i] < cl[priorIdx]) priorIdx = i;
//         if (cl[recentIdx] >= cl[priorIdx]) return false; // not a lower low, no divergence signal
//         double rsiRecent = calcRSI(Arrays.copyOfRange(cl, 0, recentIdx + 1), RSI_PERIOD);
//         double rsiPrior  = calcRSI(Arrays.copyOfRange(cl, 0, priorIdx + 1), RSI_PERIOD);
//         return rsiRecent > rsiPrior;
//     }
...
//     // =========================================================================
//     // FIX #10: Funding rate — NOTE: field name is a best-effort guess based on
//     // common exchange instrument payload conventions. Please verify the actual
//     // key against a live response from
//     // /exchange/v1/derivatives/futures/data/instrument?pair=... and adjust the
//     // optDouble key below if it differs (e.g. it may be nested, or named
//     // "funding_rate_8h" or similar).
//     // =========================================================================
//     private static Double getFundingRate(String pair) {
//         try {
//             JSONObject inst = instrumentCache.get(pair);
//             if (inst == null) return null;
//             if (!inst.has("funding_rate")) return null;
//             return inst.optDouble("funding_rate", Double.NaN);
//         } catch (Exception e) {
//             return null;
//         }
//     }
...
//     // =========================================================================
//     // FIX #14: Orderbook spread — NOTE: verify the actual response shape
//     // against the live CoinDCX orderbook endpoint; this assumes a JSON object
//     // with "bids"/"asks" maps of price-string -> quantity.
//     // =========================================================================
//     private static Double getOrderBookSpreadPercent(String pair) {
//         try {
//             HttpURLConnection conn = openGet(PUBLIC_API_URL + "/market_data/orderbook?pair=" + pair);
//             if (conn.getResponseCode() != 200) return null;
//             JSONObject r = new JSONObject(readStream(conn.getInputStream()));
//             JSONObject bids = r.optJSONObject("bids");
//             JSONObject asks = r.optJSONObject("asks");
//             if (bids == null || asks == null || bids.length() == 0 || asks.length() == 0) return null;
//             double bestBid = 0, bestAsk = Double.MAX_VALUE;
//             for (String k : bids.keySet()) bestBid = Math.max(bestBid, Double.parseDouble(k));
//             for (String k : asks.keySet()) bestAsk = Math.min(bestAsk, Double.parseDouble(k));
//             if (bestBid <= 0 || bestAsk <= 0 || bestAsk == Double.MAX_VALUE) return null;
...
//     // =========================================================================
//     // Places a LIMIT order with a small buffer instead of a market order,
//     // to reduce slippage.
//     // =========================================================================
//     public static JSONObject placeFuturesOrder(String side, String pair, double qty,
//                                                      int lev, String notif,
...
//     // Used for 2H / 4H / Daily (since CoinDCX API doesn't support those
//     // resolutions natively — derived from the extended 1H fetch).
//     // =========================================================================
//     private static JSONArray aggregateCandles(JSONArray source, int groupSize) {
//         if (source == null || source.length() < groupSize) return null;
//         int n = source.length();
...
// /**
//  * ═══════════════════════════════════════════════════════════════════════════
//  * CoinDCX Futures Trader — v14-FIXED (post loss-analysis patch)
//  * ═══════════════════════════════════════════════════════════════════════════
//  *
//  * CHANGES FROM v14 → v14-FIXED:
//  *
//  *   FIX #1 — SL ATR mismatch on 1H fallback
//  *     Before: When falling back to 1H Supertrend bands, the SL buffer and
//  *             clamp still multiplied atr30m (30m ATR), which is much smaller
//  *             than 1H noise — SL ended up unrealistically tight vs a band
//  *             computed on the bigger timeframe.
//  *     After:  A dedicated atr1h is computed. When slSource="1H", the buffer
//  *             and clamp use atr1h instead of atr30m.
//  *
//  *   FIX #2 — BTC correlation gate (G0) restored
//  *     Before: No BTC pre-check at all (removed in v13→v14).
//  *     After:  Before scanning each pair, BTC's own 1H Supertrend + EMA9/21
//  *             direction is computed once. If BTC trend conflicts with the
//  *             candidate trade direction, the trade is skipped.
//  *
//  *   FIX #3 — Market order → Limit order (reduce slippage)
//  *     Before: order_type = "market_order"
//  *     After:  order_type = "limit_order" with a small price buffer
//  *             (+0.1% for buy, -0.1% for sell) plus IOC-style handling via
//  *             existing time_in_force.
//  *
//  *   FIX #4 — Reward:Risk ratios raised
//  *     Before: RR_STRONG=1.5, RR_MEDIUM=1.4, RR_WEAK=1.3 (too low to absorb
//  *             fees + slippage reliably).
//  *     After:  RR_STRONG=2.2, RR_MEDIUM=2.0, RR_WEAK=1.8
//  *
//  *   FIX #5 — Pair universe reduced to top-40 liquid pairs
//  *     Before: ~270 pairs scanned sequentially — by the time later pairs are
//  *             reached, earlier signals are stale (GH Actions run can take
//  *             several minutes across ~270 pairs × 5 candle fetches).
//  *     After:  COIN_SYMBOLS trimmed to the 40 most liquid USDT pairs.
//  *
//  *   FIX #6 — MAX_OPEN_POSITIONS cap added
//  *     Before: No cap — every qualifying pair in a single scan could open
//  *             a position simultaneously.
//  *     After:  Scan aborts once active position count reaches
...
//     // =========================================================================
//     // FIX #5: Coin list reduced to top-40 liquid USDT pairs
//     // =========================================================================
//     private static final String[] COIN_SYMBOLS = {
//         "ETH", "SOL", "ZEC", "XRP", "DOGE", "BNB", "TAO", "1000PEPE", "ADA", "SUI",
//         "BCH", "LINK", "AVAX", "FIL", "OP", "NEAR", "TRX", "TRUMP", "ARB", "WLD",
...
//     // =========================================================================
//     // FIX #2: BTC direction gate (G0)
//     // Computes BTC's own 1H trend (Supertrend + EMA9/21). Returns:
//     //   true  -> BTC bullish
//     //   false -> BTC bearish
//     //   null  -> insufficient data (gate is skipped, not blocked)
//     // =========================================================================
//     private static Boolean getBtcDirection() {
//         try {
//             JSONArray btc1h = getCandlestickData("B-BTC_USDT", "60", CANDLE_1H);
...
//             boolean[] st = calcSupertrend(hi, lo, cl, ST_PERIOD, ST_MULTIPLIER);
//             boolean stBull = st[st.length - 1];
//             // Majority of the two signals decides BTC direction
//             if (emaBull == stBull) return emaBull;
//             // Disagreement -> lean on Supertrend (more responsive to trend flips)
//             return stBull;
//         } catch (Exception e) {
//             System.err.println("getBtcDirection: " + e.getMessage());
//             return null;
...
//         // ── FIX #6: hard cap on simultaneous open positions ──────────────────
//         if (active.size() >= MAX_OPEN_POSITIONS) {
//             System.out.println("MAX_OPEN_POSITIONS (" + MAX_OPEN_POSITIONS +
//                     ") already reached (" + active.size() + " open) — skipping scan entirely.");
//             return;
...
//         // ── FIX #2: BTC direction gate — computed once per run ───────────────
//         Boolean btcBullish = getBtcDirection();
//         System.out.println("BTC 1H direction: " +
//                 (btcBullish == null ? "UNKNOWN (gate skipped)" : (btcBullish ? "BULLISH" : "BEARISH")));
...
//         for (String pair : COINS_TO_TRADE) {
//             try {
//                 // ── FIX #6: re-check position cap every iteration ─────────────
//                 if (active.size() >= MAX_OPEN_POSITIONS) {
//                     System.out.println("MAX_OPEN_POSITIONS reached mid-scan — stopping.");
//                     break;
//                 }
...
//                 double atr30m   = calcATR(hi30, lo30, cl30, ATR_PERIOD);
//                 double atr15m   = calcATR(hi15, lo15, cl15, ATR_PERIOD);
//                 // ── FIX #1: dedicated 1H ATR for use when SL falls back to 1H bands
//                 double atr1h    = (raw1h != null && raw1h.length() >= ATR_PERIOD + 1)
//                         ? calcATR(extractHighs(raw1h), extractLows(raw1h), extractCloses(raw1h), ATR_PERIOD)
//                         : atr30m; // fallback if 1H data insufficient
...
//                 // ─────────────────────────────────────────────────────────────
//                 // MTF EMA9/21 + SUPERTREND CONFLUENCE FILTER
//                 // ─────────────────────────────────────────────────────────────
//                 System.out.println("  ── MTF EMA9/21 + Supertrend Check ────────");
...
//                 // ── FIX #2: BTC gate applied here, right after direction is known ──
//                 if (btcBullish != null && btcBullish != trendUp) {
//                     System.out.printf("  [G0-BTC] BTC=%s vs Candidate=%s → conflict — skip%n",
//                             btcBullish ? "BULL" : "BEAR", trendUp ? "BULL" : "BEAR");
//                     continue;
...
//                 // FIX #1: buffer/clamp now uses the ATR matching the band source
//                 // ─────────────────────────────────────────────────────────────
//                 double dynamicRR = adx >= 40 ? RR_STRONG : adx >= 30 ? RR_MEDIUM : RR_WEAK;
//                 System.out.printf("  Dynamic R:R = 1:%.1f (ADX=%.1f)%n", dynamicRR, adx);
...
//                 double[] stBands;
//                 String   slSource;
//                 double   slAtr; // FIX #1: ATR matching the band's timeframe
...
//                         stBands  = calcSupertrendBands(hi1h, lo1h, cl1h, ST_PERIOD, ST_MULTIPLIER);
//                         slSource = "1H";
//                         slAtr    = atr1h;   // FIX #1: use 1H ATR, not 30m
//                     } else {
//                         stBands  = new double[]{entry - 2.4 * atr1h, entry + 2.4 * atr1h};
//                         slSource = "ATR-fallback";
//                         slAtr    = atr1h;
...
//                 // ── FIX #6: keep local active-position tracking in sync ───────
//                 active.add(pair);
...
//     // =========================================================================
//     // FIX #3: Renamed from placeFuturesMarketOrder → placeFuturesOrder.
//     // Now places a LIMIT order with a small buffer instead of a market order,
//     // to reduce slippage. Buy orders are placed slightly above current price,
//     // sell orders slightly below — this still fills promptly on liquid pairs
//     // while avoiding the worst of orderbook-walking slippage a pure market
//     // order can suffer during volatile moments.
//     // =========================================================================
//     public static JSONObject placeFuturesOrder(String side, String pair, double qty,
//                                                      int lev, String notif,
//                                                      String marginType, String marginCcy,
