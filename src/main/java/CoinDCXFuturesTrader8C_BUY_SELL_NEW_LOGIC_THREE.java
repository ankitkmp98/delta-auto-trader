...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * CHANGE LOG (this version) — SL anchor moved 2H → 30M, tighter SL
 * ═══════════════════════════════════════════════════════════════════════════
 *   - SL is now computed off the 30M Supertrend band + ATR (was 2H). 30M
 *     naturally has a smaller ATR / tighter Supertrend band than 2H, so the
 *     stop distance shrinks without touching the entry-direction logic
 *     (4H/2H/1H/30M alignment + 15M pullback+rejection is untouched).
 *   - SL_ATR_BUFFER: 0.35 → 0.20 (smaller cushion around the ST line)
 *   - SL_MAX_PERCENT: 4.5% → 2.5% (hard cap tightened — was ~36% margin risk
 *     at 8x leverage, now ~20%)
 *   - TP still derives from RR_TARGET × risk, so TP also shrinks proportionally
 *     (R:R ratio itself is unchanged) — trades should resolve faster either way.
 *   - Swing-low/high lookback for the "extend past recent swing" rule now
 *     reads 30M candles instead of 2H candles (SWING_LOOKBACK=20 → 20×30min
 *     = 10 hours of lookback, was 20×2H = 40 hours).
 *   - ensureTpSlForOpenPositions() (safety sweep) now also fetches 30M data
 *     and uses the same 30M-anchored formula, so sweep-protected positions
 *     stay consistent with normal entries.
 * ═══════════════════════════════════════════════════════════════════════════
 */
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    private static final double SL_ATR_BUFFER   = 0.20;  // was 0.35
    private static final double SL_MAX_PERCENT  = 0.3;   // was 4.5
    ...
    private static final double RR_TARGET = 21.0;
    ...
    // NOTE: now called with 30M hi/lo arrays (was 2H) — see computeSlTp() below.
    ...
    // =========================================================================
    // unified SL/TP formula — SL ANCHOR CHANGED: 2H → 30M.
    //   SL = 30M Supertrend line ± ATR buffer (30M ATR), extended past recent
    //        30M swing low/high "if practical", capped by SL_MAX_PERCENT.
    //   TP = entry ± RR_TARGET * risk
    //
    // Parameter renamed tf30m (was tf2h) to make the anchor timeframe change
    // explicit. Struct is generic (TFResult), so only call sites needed
    // updating — see main() and ensureTpSlForOpenPositions().
    // =========================================================================
    private static double[] computeSlTp(boolean isLong, double entryPrice, TFResult tf30m, double tickSize) {
        double sl, tp;
        if (isLong) {
            double raw = tf30m.stBands[0] - SL_ATR_BUFFER * tf30m.atr;
            if (raw >= entryPrice) raw = entryPrice - (SL_ATR_BUFFER + 1.5) * tf30m.atr;
            double hardFloor = entryPrice * (1 - SL_MAX_PERCENT / 100.0);

            double swingLow = findSwingLow(tf30m.lo, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
            if (swingLow < raw && swingLow > hardFloor) {
                raw = swingLow - SWING_EXTRA_BUFFER_ATR * tf30m.atr;
            }
            ...
            double raw = tf30m.stBands[1] + SL_ATR_BUFFER * tf30m.atr;
            if (raw <= entryPrice) raw = entryPrice + (SL_ATR_BUFFER + 1.5) * tf30m.atr;
            double hardCeil = entryPrice * (1 + SL_MAX_PERCENT / 100.0);

            double swingHigh = findSwingHigh(tf30m.hi, SWING_LOOKBACK, SWING_EXCLUDE_RECENT);
            if (swingHigh > raw && swingHigh < hardCeil) {
                raw = swingHigh + SWING_EXTRA_BUFFER_ATR * tf30m.atr;
            }
    ...
    public static void main(String[] args) {
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                // ── SL/TP now anchored on 30M (was 2H) ─────────────────────────
                double[] slTp = computeSlTp(trendUp, entry, tf30m, tickSize);
                ...
                System.out.printf("  SL=%.6f (%.2f%%) | TP=%.6f (%.2f%%) | R:R target=%.2f%n",
    ...
    // =========================================================================
    // Safety sweep — now fetches 30M data too and uses the SAME 30M-anchored
    // computeSlTp() formula as the main entry path, so fallback-protected
    // positions get identical (tighter) SL/TP logic.
    // =========================================================================
    private static void ensureTpSlForOpenPositions() {
        try {
            ...
            for (String pair : stillOpen) {
                ...
                double[] slTp = computeSlTp(isLong, avgPrice, tf30m, tick);
                ...
                if (posId != null) {
                    System.out.printf("  [SWEEP] %s fallback SL=%.6f TP=%.6f (R:R target=%.2f)%n", pair, sl, tp, RR_TARGET);
    ...
    // =========================================================================
    // BigDecimal-exact tick rounding — avoids "Price should be divisible by
    // X" rejections caused by binary floating-point noise (e.g.
    // 0.026099999999999998 instead of 0.0261). Use roundToTickBD() wherever a
    // price is about to go into a JSON payload sent to the exchange.
    // =========================================================================
