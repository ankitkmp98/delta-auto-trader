...
// are UNCHANGED. Summary of what changed this pass:
//
//   New 4-timeframe hierarchy, replacing the old 1H->30M->5M cascade:
//     30M  -> Main Trend        (EMA9/21 + price + Supertrend, decides direction)
//     15M  -> Trend Confirmation (same condition set, must agree with 30M)
//     5M   -> Momentum / Setup  (same condition set + ADX(14)>20 + Volume>Avg20)
//     3M   -> Entry Timing      (RSI(14) vs 50, pullback to EMA9/EMA21,
//                                 rejection candle, overextension filter)
//   All layers are MANDATORY (no soft/optional gates this time — rejection
//   candle in particular is now required again, since it IS the entry
//   trigger in this design, not just a confirmation).
//
//   SL/TP formula itself is unchanged (Supertrend +/- SL_ATR_BUFFER x ATR,
//   TP at RR_TARGET) but now sourced from 3M data, since 3M is the new
//   lowest/entry timeframe (was 5M before).
//
//   New helper added: calcRSI() (Wilder RSI), didn't exist before.
// =========================================================================
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // Pullback distance (on 3M, to EMA9 or EMA21 per the new design).
    private static final double PULLBACK_MAX_ATR = 0.9;

    // SL buffer applied against 3M ATR now (was 5M before).
    private static final double SL_ATR_BUFFER = 0.4;

    private static final double RR_TARGET = 2.0; // fixed Risk:Reward

    private static final double LIMIT_ORDER_BUFFER_PCT = 0.001;

    // === STRATEGY UPDATE: new 4-timeframe cascade — 30M(main)/15M(confirm)/
    // 5M(momentum)/3M(entry). Candle counts sized to comfortably clear
    // analyzeTF()'s minimum (EMA_MID + ST_PERIOD + 5 = 36 bars) and give
    // calcADX()/calcRSI() a stable read.
    private static final int CANDLE_30M = 100;
    private static final int CANDLE_15M = 100;
    private static final int CANDLE_5M  = 100;
    private static final int CANDLE_3M  = 150;

    private static final int    ADX_PERIOD    = 14;
    private static final double ADX_THRESHOLD = 20.0;
    private static final int    VOLUME_EMA_PERIOD = 20;

    // === STRATEGY UPDATE: new — RSI(14) momentum confirmation on 3M.
    // Long needs RSI > 50, Short needs RSI < 50 (momentum agrees with
    // direction) — deliberately NOT used as an overbought/oversold
    // reversal signal (no 70/30 logic), per the design brief.
    private static final int    RSI_PERIOD     = 14;
    private static final double RSI_MID        = 50.0;

    // === STRATEGY UPDATE: new — overextension filter on 3M. If the entry
    // candle's range is more than this multiple of ATR(14), skip — avoids
    // entering at the top of a pump / bottom of a dump.
    private static final double OVEREXTENSION_ATR_MULT = 1.5;

    // Rejection candle is the actual entry trigger in this design, so it's
    // mandatory again (no toggle to disable it this time).
    private static final boolean REQUIRE_REJECTION_CANDLE = true;

    // === Risk-based position sizing config (unchanged) ===
    ...
    private static TFResult analyzeTF(JSONArray candles) {
        ...
        // Trend layer = EMA9/21 cross + price vs both EMAs + Supertrend colour.
        // Same condition set reused identically at every timeframe in the
        // new hierarchy (30M/15M/5M), per the design brief.
        r.bullish = r.stGreen  && (r.ema9 > r.ema21) && priceAboveEmas;
        r.bearish = (!r.stGreen) && (r.ema9 < r.ema21) && priceBelowEmas;
        return r;
    }
    ...
    // === STRATEGY UPDATE: SL/TP formula unchanged — now sourced from 3M
    // Supertrend + ATR, since entry timing moved to 3M (bottom of the new
    // 30M -> 15M -> 5M -> 3M hierarchy).
    // stBandsTF = {lowerBand, upperBand} from calcSupertrendBands() on 3M data,
    // atrTF = calcATR(14) on 3M data. Used both by the live scan (real entry
    // or a pre-fill current-price estimate) and by the safety sweep.
    //
    //   Long:  SL = 3M-Supertrend lower band - (SL_ATR_BUFFER x ATR3)
    //          TP = Entry + (Entry - SL) x RR_TARGET
    //   Short: SL = 3M-Supertrend upper band + (SL_ATR_BUFFER x ATR3)
    ...
    public static void main(String[] args) {
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                JSONArray raw3m  = getCandlestickData(pair, "3",  CANDLE_3M);

                // === STRATEGY UPDATE: 30M = Main Trend (MANDATORY, decides direction) ===
                ...
                boolean trendUp;
                if (tf30m.bullish) trendUp = true;
                else if (tf30m.bearish) trendUp = false;
                else { System.out.println("  30M FAIL — main trend not clean — skip"); continue; }

                // === STRATEGY UPDATE: 15M = Trend Confirmation (MANDATORY, agree with 30M) ===
                ...
                System.out.printf("  [15M] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        ...
                        tf15mAligned ? "ALIGNED" : "NOT ALIGNED");
                if (!tf15mAligned) {
                    System.out.println("  15M FAIL — disagrees with 30M trend — skip"); continue;
                }
                System.out.println("  30M+15M OK — " + (trendUp ? "BULLISH" : "BEARISH") + " confirmed on both");

                // === STRATEGY UPDATE: 5M = Momentum/Setup (MANDATORY) ===
                // Same trend condition set as 30M/15M, PLUS ADX(14) > 20, PLUS
                // current volume > 20-period volume EMA (participation).
                ...
                System.out.printf("  [5M] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        ...
                        tf5mAligned ? "ALIGNED" : "NOT ALIGNED");
                if (!tf5mAligned) {
                    System.out.println("  5M FAIL — disagrees with 30M/15M trend — skip"); continue;
                }

                double adx5 = calcADX(tf5m.hi, tf5m.lo, tf5m.cl, ADX_PERIOD);
                ...
                if (!adxOk) {
                    System.out.println("  5M FAIL — ADX below threshold, momentum too weak — skip"); continue;
                }

                double[] vol5   = extractVolumes(raw5m);
                double volEma5  = calcEMA(vol5, VOLUME_EMA_PERIOD);
                double currentVol5 = vol5[vol5.length - 1];
                boolean volOk = currentVol5 > volEma5;
                System.out.printf("  [5M-Volume] current=%.2f | EMA%d=%.2f → %s%n",
                        currentVol5, VOLUME_EMA_PERIOD, volEma5, volOk ? "PASS" : "FAIL");
                if (!volOk) {
                    System.out.println("  5M FAIL — volume below 20-period volume EMA — skip"); continue;
                }
                System.out.println("  5M OK — momentum + participation confirmed");

                // === STRATEGY UPDATE: 3M = Entry Timing (MANDATORY) ===
                ...
                int n3 = cl3.length;
                if (n3 < 3) { System.out.println("  Not enough 3m candles for entry check — skip"); continue; }

                double entryClose = cl3[n3 - 2], entryOpen = op3[n3 - 2];
                double entryHigh  = hi3[n3 - 2], entryLow  = lo3[n3 - 2];
                double prevClose  = cl3[n3 - 3], prevOpen  = op3[n3 - 3];
                ...
                boolean longOk3Trend  = ema9_3 > ema21_3 && entryClose > ema9_3 && entryClose > ema21_3 && stBull3;
                boolean shortOk3Trend = ema9_3 < ema21_3 && entryClose < ema9_3 && entryClose < ema21_3 && !stBull3;
                boolean tf3TrendOk = trendUp ? longOk3Trend : shortOk3Trend;
                System.out.printf("  [3M] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        stBull3 ? "GREEN" : "RED", ema9_3, ema21_3, entryClose,
                        tf3TrendOk ? "ALIGNED" : "NOT ALIGNED");
                if (!tf3TrendOk) {
                    System.out.println("  3M FAIL — trend/Supertrend filter not satisfied — skip"); continue;
                }

                // RSI momentum confirmation — NOT overbought/oversold, just
                // "is momentum on the same side as the trade".
                double rsi3 = calcRSI(cl3, RSI_PERIOD);
                boolean rsiOk = trendUp ? rsi3 > RSI_MID : rsi3 < RSI_MID;
                System.out.printf("  [3M-RSI] RSI(%d)=%.2f (need %s %.1f) → %s%n",
                        RSI_PERIOD, rsi3, trendUp ? ">" : "<", RSI_MID, rsiOk ? "PASS" : "FAIL");
                if (!rsiOk) {
                    System.out.println("  3M FAIL — RSI momentum disagrees with trade direction — skip"); continue;
                }

                // Pullback to EMA9 or EMA21 (Supertrend line no longer used for
                // pullback distance in this design — trend/ST is already
                // confirmed above, pullback here is purely EMA-based per spec).
                ...
                boolean pullbackOk = distEma9 <= maxDist || distEma21 <= maxDist;
                System.out.printf("  [3M-Pullback] distEMA9=%.6f distEMA21=%.6f maxAllowed=%.6f → %s%n",
                        distEma9, distEma21, maxDist, pullbackOk ? "PASS" : "FAIL");
                if (!pullbackOk) {
                    System.out.println("  3M FAIL — no valid pullback to EMA9/EMA21 — skip"); continue;
                }

                // Overextension filter — skip if the entry candle's range is
                // too large relative to ATR (avoids buying a pump top / selling
                // a dump bottom).
                double candleRange = entryHigh - entryLow;
                double maxRange = OVEREXTENSION_ATR_MULT * atr3;
                boolean notOverextended = candleRange <= maxRange;
                System.out.printf("  [3M-Overextension] candleRange=%.6f maxAllowed=%.6f (%.1fxATR) → %s%n",
                        candleRange, maxRange, OVEREXTENSION_ATR_MULT, notOverextended ? "PASS" : "FAIL");
                if (!notOverextended) {
                    System.out.println("  3M FAIL — entry candle overextended vs ATR — skip"); continue;
                }

                // Rejection candle — the actual entry trigger (mandatory).
                ...
                System.out.println("  3M OK — trend + RSI + pullback + overextension + rejection all confirmed");
                ...
                // the same 3M-Supertrend + ATR3 buffer that the real SL will use.
                // calcQuantityByRisk() itself is unchanged from the previous version.
                double[] preSlTp = computeSlTp(trendUp, currentPrice, stBands3, atr3, tickSize);
                ...
                double[] slTp = computeSlTp(trendUp, entry, stBands3, atr3, tickSize);
    ...
    private static void ensureTpSlForOpenPositions() {
        try {
            ...
            for (String pair : stillOpen) {
                ...
                // === STRATEGY UPDATE: sweep now derives SL/TP from 3M Supertrend + ATR,
                // same basis as the live scan (was 5M before).
                JSONArray raw3mSweep = getCandlestickData(pair, "3", CANDLE_3M);
                if (raw3mSweep == null || raw3mSweep.length() < EMA_MID + 5) {
                    System.out.println("  [SWEEP] insufficient 3M data for " + pair + " — will retry next run");
                    continue;
                }
                double[] hi3Sweep = extractHighs(raw3mSweep);
                double[] lo3Sweep = extractLows(raw3mSweep);
                double[] cl3Sweep = extractCloses(raw3mSweep);
                double atr3Sweep = calcATR(hi3Sweep, lo3Sweep, cl3Sweep, ATR_PERIOD);
                double[] stBands3Sweep = calcSupertrendBands(hi3Sweep, lo3Sweep, cl3Sweep, ST_PERIOD, ST_MULTIPLIER);

                double posQty = pos.optDouble("active_pos", 0);
                boolean isLong = posQty >= 0;

                double tick = getTickSize(pair);
                double[] slTp = computeSlTp(isLong, avgPrice, stBands3Sweep, atr3Sweep, tick);
    ...
    // Wilder ADX(14) — +DM/-DM/TR Wilder smoothing, DX = 100*|+DI--DI|/(+DI+-DI),
    ...
    // === STRATEGY UPDATE: new helper — Wilder RSI(14), didn't exist before.
    // Standard Wilder-smoothed average gain/loss, RSI = 100 - 100/(1+RS).
    // Used purely as a momentum-direction confirmation (RSI vs 50), not as
    // an overbought/oversold reversal signal.
    ...
    // Risk-based sizing (unchanged): Position Size = (Account Balance x Risk%) / |Entry - SL|
    // (converted from INR risk amount to coin qty via USDT->INR rate). Does NOT
    // divide by leverage — leverage changes required margin, not INR-at-risk
    // for a given qty and SL distance. Old MAX_MARGIN kept as a hard safety
    // ceiling on qty.
    ...
    // Order cancellation — a good_till_cancel limit order that never fills
    // within the entry-confirm window gets cancelled here instead of being
    // left open/pending forever.
