...
/**
 * =============================================================================
 * SIMPLIFIED SCALPING BOT — per the "keep it simple" spec:
 *   5M  -> direction bias only (EMA9 vs EMA21, price vs EMA9, SuperTrend)
 *   1M  -> entry trigger only (same 3 conditions, on the CLOSED 1M candle)
 *   SL  -> 5M SuperTrend line ± a small configurable % buffer
 *   TP  -> fixed Risk:Reward multiple (RR_TARGET) off that SL
 *   Exit -> TP hit OR SL hit ONLY (both set as exchange-native triggers at
 *           entry time). Nothing else. No SuperTrend-flip exit, no trailing,
 *           no partial booking, no early-exit score system, no 15M/30M/3M
 *           cascade, no ADX/RSI/volume filters.
 *   Sizing -> fixed margin notional per trade (MAX_MARGIN_CAP), same as the
 *             original bot's default behaviour — SL distance does NOT change
 *             position size here.
 *
 * Everything below EMA/SuperTrend/SL/TP/exit logic (API auth, candle
 * fetching, order placement, TP/SL setting, position polling, instrument
 * cache, tick rounding) is carried over unchanged from the existing bot's
 * architecture.
 * =============================================================================
 */
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    private static final String RES_1M = "1";

    // Just enough candles for EMA21 + SuperTrend(10) to be stable.
    private static final int FETCH_COUNT_5M = 120;
    private static final int FETCH_COUNT_1M = 120;

    // =========================================================================
    // SL: 5M SuperTrend line ± this % buffer of entry price.
    // Configurable, per spec section 8/9 — start small (0.05%-0.10%).
    // =========================================================================
    private static final double SUPERTREND_SL_BUFFER_PERCENT = 0.08; // 0.08%
    ...
    private static final double SL_MIN_PERCENT = 0.45; // never tighter than this
    private static final double SL_MAX_PERCENT = 0.80;  // never wider than this

    // =========================================================================
    // TP: fixed Risk:Reward multiple off the SL distance. Configurable.
    // =========================================================================
    private static final double RR_TARGET = 1.8; // 1.5R initially; try 1.75/2.0 later

    // =========================================================================
    // Fixed-margin sizing (unchanged behaviour from the existing bot) — SL
    // distance does NOT affect position size, every trade uses the same
    // notional cap.
    // =========================================================================
    private static final double MAX_MARGIN_CAP = 1200.0; // INR notional cap per trade @ leverage
    ...
    // =========================================================================
    // Per-position tracking — only what's needed for logging + the
    // SuperTrend-flip exit check. No trailing state, no stages, no partials.
    // =========================================================================
    ...
    private static final Map<String, PositionInfo> positionState = new ConcurrentHashMap<>();

    // Duplicate-entry protection: last CLOSED 1M candle timestamp we already
    // acted on for a given pair, so the same candle never fires two entries.
    private static final Map<String, Long> lastProcessedEntryCandleTimestamp = new ConcurrentHashMap<>();
    ...
    // =========================================================================
    // Direction / trigger result types
    // =========================================================================
    private enum Direction { LONG, SHORT, SKIP }
    ...
    private static class DirectionResult {
        ...
        double ema9, ema21;
        boolean stGreen;
        double stLineValue; // 5M SuperTrend active band value — this is what SL is anchored to now
        String reason;
    }
    ...
    private static class TriggerResult {
        boolean valid;
        boolean triggered;
        double ema9, ema21, close;
        boolean stGreen;
        double stLineValue;     // the ACTIVE SuperTrend band value on the closed 1M candle
        long   candleTimestamp; // used for duplicate-entry protection
        String reason;
    }

    // =========================================================================
    // 5M DIRECTION BIAS — exactly 3 conditions, on the closed 5M candle.
    // LONG:  EMA9 > EMA21   AND close > EMA9   AND SuperTrend GREEN
    // SHORT: EMA9 < EMA21   AND close < EMA9   AND SuperTrend RED
    // Anything else -> SKIP.
    // =========================================================================
    private static DirectionResult get5MDirection(JSONArray candles5m) {
        DirectionResult r = new DirectionResult();
        int minBars = EMA_SLOW + ST_PERIOD + 5;
        if (candles5m == null || candles5m.length() < minBars) {
            r.valid = false;
            return r;
        }
        double[] cl = extractCloses(candles5m);
        double[] hi = extractHighs(candles5m);
        double[] lo = extractLows(candles5m);

        double ema9  = calcEMA(cl, EMA_FAST);
        double ema21 = calcEMA(cl, EMA_SLOW);
        double close = cl[cl.length - 1];
        SupertrendState st = calcSupertrendState(hi, lo, cl, ST_PERIOD, ST_MULTIPLIER);

        r.valid = true;
        r.ema9 = ema9; r.ema21 = ema21; r.stGreen = st.bullish;
        r.stLineValue = st.lineValue;

        boolean longOk  = ema9 > ema21 && close > ema9 && st.bullish;
        boolean shortOk = ema9 < ema21 && close < ema9 && !st.bullish;

        if (longOk) {
            r.direction = Direction.LONG;
            r.reason = String.format("5M LONG: EMA9(%.6f)>EMA21(%.6f), close(%.6f)>EMA9, ST=GREEN", ema9, ema21, close);
        } else if (shortOk) {
            r.direction = Direction.SHORT;
            r.reason = String.format("5M SHORT: EMA9(%.6f)<EMA21(%.6f), close(%.6f)<EMA9, ST=RED", ema9, ema21, close);
        } else {
            r.direction = Direction.SKIP;
            r.reason = String.format("5M SKIP: EMA9=%.6f EMA21=%.6f close=%.6f ST=%s (conditions not aligned)",
                    ema9, ema21, close, st.bullish ? "GREEN" : "RED");
        }
        return r;
    }

    // =========================================================================
    // 1M ENTRY TRIGGER — same 3 conditions, on the closed 1M candle, gated by
    // the already-decided 5M direction. Also returns the active SuperTrend
    // line value (needed for SL placement) and the candle's own timestamp
    // (needed for duplicate-entry protection).
    // =========================================================================
    private static TriggerResult get1MTrigger(JSONArray candles1m, Direction direction5m) {
        TriggerResult r = new TriggerResult();
        int minBars = EMA_SLOW + ST_PERIOD + 5;
        if (candles1m == null || candles1m.length() < minBars || direction5m == Direction.SKIP) {
            r.valid = false;
            return r;
        }
        double[] cl = extractCloses(candles1m);
        double[] hi = extractHighs(candles1m);
        double[] lo = extractLows(candles1m);
        int n = cl.length;
        ...
        r.valid = true;
        r.ema9 = ema9; r.ema21 = ema21; r.close = close; r.stGreen = st.bullish;
        r.stLineValue = st.lineValue;
        r.candleTimestamp = candles1m.getJSONObject(n - 1).optLong("time", 0);

        boolean longOk  = direction5m == Direction.LONG  && ema9 > ema21 && close > ema9 && st.bullish;
        boolean shortOk = direction5m == Direction.SHORT && ema9 < ema21 && close < ema9 && !st.bullish;

        r.triggered = longOk || shortOk;
        r.reason = String.format("1M: EMA9=%.6f EMA21=%.6f close=%.6f ST=%s ST_line=%.6f -> triggered=%s",
                ema9, ema21, close, st.bullish ? "GREEN" : "RED", st.lineValue, r.triggered);
        return r;
    }

    // =========================================================================
    // Section 17 — combined decision function.
    // =========================================================================
    private static class ScalpSignal {
        Direction direction = Direction.SKIP;
        boolean triggered;
        double stLineValue5m; // SL anchor — 5M SuperTrend active band value
        double close1m;
        long candleTimestamp1m;
        String logLine;
    }

    private static ScalpSignal analyzeScalpingSignal(String symbol, JSONArray candles5m, JSONArray candles1m) {
        ScalpSignal s = new ScalpSignal();
        DirectionResult dir5m = get5MDirection(candles5m);
        if (!dir5m.valid) {
            s.logLine = symbol + " — insufficient 5M data";
            return s;
        }
        s.direction = dir5m.direction;
        s.stLineValue5m = dir5m.stLineValue;

        StringBuilder log = new StringBuilder();
        log.append(symbol).append("\n  5M: EMA9=").append(String.format("%.6f", dir5m.ema9))
           .append(" EMA21=").append(String.format("%.6f", dir5m.ema21))
           .append(" ST=").append(dir5m.stGreen ? "GREEN" : "RED")
           .append(" ST_line=").append(String.format("%.6f", dir5m.stLineValue))
           .append(" -> Direction=").append(dir5m.direction);

        if (dir5m.direction == Direction.SKIP) {
            log.append("\n  Decision=SKIP  Reason=5M direction not aligned");
            s.logLine = log.toString();
            return s;
        }

        TriggerResult trig1m = get1MTrigger(candles1m, dir5m.direction);
        if (!trig1m.valid) {
            log.append("\n  Decision=SKIP  Reason=insufficient 1M data");
            s.logLine = log.toString();
            return s;
        }

        log.append("\n  1M: EMA9=").append(String.format("%.6f", trig1m.ema9))
           .append(" EMA21=").append(String.format("%.6f", trig1m.ema21))
           .append(" ST=").append(trig1m.stGreen ? "GREEN" : "RED")
           .append(" Close=").append(String.format("%.6f", trig1m.close));

        s.triggered = trig1m.triggered;
        s.close1m = trig1m.close;
        s.candleTimestamp1m = trig1m.candleTimestamp;

        if (trig1m.triggered) {
            log.append("\n  Decision=").append(dir5m.direction)
               .append("  Reason=5M ").append(dir5m.direction).append(" + 1M ").append(dir5m.direction).append(" confirmation");
        } else {
            log.append("\n  Decision=SKIP  Reason=5M trend is ").append(dir5m.direction)
               .append(" but 1M did not confirm (").append(trig1m.reason).append(")");
        }
        s.logLine = log.toString();
        return s;
    }
    ...
    // =========================================================================
    // SL/TP construction — SL is the 5M SuperTrend line ± a small % buffer.
    // TP is a fixed RR multiple off the resulting risk. Percent floor/ceiling
    // only guards against a broken/degenerate ST value; it never overrides a
    // sane SuperTrend-based SL.
    // =========================================================================
    private static double[] computeSlTp(boolean isLong, double entryPrice, double stLineValue, double tickSize) {
        double buffer = entryPrice * (SUPERTREND_SL_BUFFER_PERCENT / 100.0);
        double sl = isLong ? stLineValue - buffer : stLineValue + buffer;

        // Guard: SL must sit on the correct side of entry.
        boolean sideValid = isLong ? sl < entryPrice : sl > entryPrice;
        if (!sideValid) {
            sl = isLong ? entryPrice * (1 - SL_MIN_PERCENT / 100.0) : entryPrice * (1 + SL_MIN_PERCENT / 100.0);
        }

        // Guard: clamp into a sane [MIN%, MAX%] band in case the ST value is
        // unusually close/far (e.g. right after a fresh flip).
        ...
        double tp = isLong ? entryPrice + RR_TARGET * risk : entryPrice - RR_TARGET * risk;
    ...
    private static double calcQuantity(double price, String pair) {
        double qty = MAX_MARGIN_CAP / (price * USDT_INR_RATE);
    ...
    public static void main(String[] args) {
        System.out.println("=== Simplified Scalping Bot starting (5M direction + 1M trigger, "
                + "SL=5M SuperTrend±buffer, TP=" + RR_TARGET + "R, exit on TP/SL hit only — exchange-native) ===");
        initInstrumentCache();
    ...
    private static void runScan() {
        ...
        positionState.keySet().removeIf(p -> !active.contains(p));

        // ---- Ensure TP/SL is set on any position missing it (safety net) ----
        // This is now the ONLY thing monitoring open positions — TP/SL are
        // exchange-native triggers, so once they're set the exchange closes
        // the position itself. No manual flip-exit check anymore.
        ensureTpSlForOpenPositions(active);

        if (active.size() >= MAX_OPEN_POSITIONS) {
            System.out.println("MAX_OPEN_POSITIONS reached — skipping new entries this cycle.");
            return;
        }
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                if (active.contains(pair)) continue; // one position per symbol

                long lastTrade = lastTradeTime.getOrDefault(pair, 0L);
                if (System.currentTimeMillis() - lastTrade < SCALP_COOLDOWN_MS) continue;

                JSONArray raw5m = dropLastIfForming(getCandlestickData(pair, RES_5M, FETCH_COUNT_5M));
                if (raw5m == null) continue;

                DirectionResult dir5mQuick = get5MDirection(raw5m);
                if (!dir5mQuick.valid || dir5mQuick.direction == Direction.SKIP) continue; // cheap pre-filter, no 1M fetch needed

                JSONArray raw1m = dropLastIfForming(getCandlestickData(pair, RES_1M, FETCH_COUNT_1M));
                if (raw1m == null) continue;

                ScalpSignal signal = analyzeScalpingSignal(pair, raw5m, raw1m);
                System.out.println("  " + signal.logLine);

                if (signal.direction == Direction.SKIP || !signal.triggered) continue;

                // Duplicate-entry protection: don't act twice on the same closed 1M candle.
                Long lastCandle = lastProcessedEntryCandleTimestamp.get(pair);
                if (lastCandle != null && lastCandle == signal.candleTimestamp1m) continue;

                tryEnter(pair, signal, active);
    ...
    private static void tryEnter(String pair, ScalpSignal signal, Set<String> active) {
        try {
            boolean isLong = signal.direction == Direction.LONG;
            ...
            System.out.println("\n==== " + pair + " — " + signal.direction + " ENTRY (1M candle triggered) ====");
            ...
            lastProcessedEntryCandleTimestamp.put(pair, signal.candleTimestamp1m);

            double entry = getEntryPrice(pair, orderResp.getString("id"));
            if (entry <= 0) {
                System.out.println("  Could not confirm entry within window — TP/SL handled by safety sweep next cycle");
                active.add(pair);
                return;
            }
            System.out.printf("  Entry confirmed: %.6f%n", entry);

            // Re-derive SL/TP off the ACTUAL fill price, using the same 5M
            // SuperTrend line value captured at signal time.
            double[] slTp = computeSlTp(isLong, entry, signal.stLineValue5m, tickSize);
            double slPrice = slTp[0], tpPrice = slTp[1];

            System.out.println("[ENTRY] " + pair + " " + signal.direction
                    + " Entry=" + entry + " SL=" + slPrice + " TP=" + tpPrice
                    + " Risk=" + Math.abs(entry - slPrice) + " RR=" + RR_TARGET
                    + " (SL = 5M ST line " + signal.stLineValue5m + " " + (isLong ? "-" : "+") + " "
                    + SUPERTREND_SL_BUFFER_PERCENT + "% buffer)");
            ...
            pi.slPrice = slPrice;
            pi.tpPrice = tpPrice;
            positionState.put(pair, pi);

            active.add(pair);
        } catch (Exception e) {
            System.err.println("tryEnter(" + pair + "): " + e.getMessage());
        }
    }

    // Fallback only — sets a % based SL/TP if for some reason a position is
    // open without TP/SL set (order fill confirmed but the create_tpsl call
    // never got through). Never overwrites TP/SL that already exists.
    private static void ensureTpSlForOpenPositions(Set<String> active) {
        for (String pair : active) {
            try {
                ...
                double sl, tp;
                PositionInfo pi = positionState.get(pair);
                if (pi != null && pi.slPrice > 0 && pi.tpPrice > 0) {
                    sl = pi.slPrice; tp = pi.tpPrice;
                } else {
                    // No tracked state (likely a restart) — recompute from a fresh 5M SuperTrend read.
                    JSONArray raw5m = dropLastIfForming(getCandlestickData(pair, RES_5M, FETCH_COUNT_5M));
                    if (raw5m != null && raw5m.length() >= EMA_SLOW + ST_PERIOD + 5) {
                        double[] cl = extractCloses(raw5m), hi = extractHighs(raw5m), lo = extractLows(raw5m);
                        SupertrendState st = calcSupertrendState(hi, lo, cl, ST_PERIOD, ST_MULTIPLIER);
                        double[] slTp = computeSlTp(isLong, avgPrice, st.lineValue, tick);
                        sl = slTp[0]; tp = slTp[1];
                    } else {
                        // Absolute last resort — flat % fallback.
                        sl = isLong ? avgPrice * (1 - SL_MAX_PERCENT / 100.0) : avgPrice * (1 + SL_MAX_PERCENT / 100.0);
                        tp = isLong ? avgPrice + RR_TARGET * (avgPrice - sl) : avgPrice - RR_TARGET * (sl - avgPrice);
                    }
                }
                ...
                if (posId != null) {
                    ...
                    if (pi == null) {
                        ...
                        npi.isLong = isLong; npi.entryPrice = avgPrice; npi.slPrice = sl; npi.tpPrice = tp;
                        positionState.put(pair, npi);
                    }
                }
            } catch (Exception e) {
                System.err.println("ensureTpSlForOpenPositions(" + pair + "): " + e.getMessage());
            }
        }
    }
    ...
    // =========================================================================
    // SuperTrend — returns both direction AND the current bar's active band
    // value (the "SuperTrend line" the SL is anchored to).
    // =========================================================================
    ...
    private static SupertrendState calcSupertrendState(double[] hi, double[] lo, double[] cl,
                                                         int period, double multiplier) {
        SupertrendState state = new SupertrendState();
        int n = cl.length;
        if (n < period + 1) {
            state.bullish = true;
            state.lineValue = cl[n - 1];
            return state;
        }
        ...
        for (int i = period; i < n; i++) {
            ...
            if (i == period) {
                ...
                upperBand[i] = (basicUpper < upperBand[i-1] || cl[i-1] > upperBand[i-1]) ? basicUpper : upperBand[i-1];
                lowerBand[i] = (basicLower > lowerBand[i-1] || cl[i-1] < lowerBand[i-1]) ? basicLower : lowerBand[i-1];
            }
            if (i == period) bullish[i] = cl[i] > (hi[i] + lo[i]) / 2.0;
            else bullish[i] = bullish[i-1] ? cl[i] >= lowerBand[i] : cl[i] > upperBand[i];
        }
        state.bullish = bullish[n - 1];
        state.lineValue = state.bullish ? lowerBand[n - 1] : upperBand[n - 1];
        return state;
    }
    ...
    private static JSONArray getCandlestickData(String pair, String resolution, int count) {
        try {
            long minsPerBar = "1".equals(resolution) ? 1 : "5".equals(resolution) ? 5 : 15;
...
//     // =========================================================================
//     // NEW — 1-minute candles, aggregated into 3M bars for the new entry
//     // trigger timeframe. CoinDCX's documented candlestick resolutions are
//     // only '1', '5', '60', '1D' — there is no native 3-minute resolution —
//     // so 3M bars are built here from 3 consecutive 1M candles rather than
//     // (incorrectly) treating 5M candles as 3M.
//     // =========================================================================
//     private static final String RES_1M = "1";
//     private static final int BASE_1M_FETCH_COUNT = 200; // ~66 x 3M bars after aggregation
//     private static final int GROUP_3M_FROM_1M = 3;
...
//     // LEGACY — no longer used to gate entries (1H removed from the entry
//     // decision entirely; 30M direction now uses scoreDirection30M() below
//     // instead of analyzeScored6()). Left in place only because
//     // checkTrendValidForExtension() still uses SETUP_15M_MIN_SCORE via
//     // analyzeSetup15M() for the (unchanged) TP-extension gate.
//     // =========================================================================
...
//     // =========================================================================
//     // 15M setup, scored out of 5. LEGACY for entry purposes — still used
//     // as-is by checkTrendValidForExtension() (TP extension gate), which we
//     // are not changing. The NEW 15M entry-confirmation logic uses
//     // scoreConfirm15M()/CONFIRM_15M_MIN below instead.
//     // =========================================================================
//     private static final int SETUP_15M_MIN_SCORE = 3;

//     // =========================================================================
//     // NEW — 30M direction (primary trend-direction timeframe, replacing the
//     // old 1H mandatory gate + 6-point 30M confirmation). Only 4 conditions:
//     // EMA9 vs EMA21, price vs EMA21, Supertrend, EMA9 slope. Deliberately
//     // loose — 30M's only job is to establish the dominant intraday
//     // direction, not to wait for a perfect trend.
//     //   DIRECTION_30M_STRONG_MIN = 3/4 -> confident direction, accepted outright
//     //   DIRECTION_30M_WEAK_MIN   = 2/4 -> accepted ONLY if the opposite side
//     //       scores <=1 (i.e. no real conflict) — a genuine 2-vs-2 split is
//     //       treated as "clear conflict" and skipped.
//     // =========================================================================
//     private static final int DIRECTION_30M_STRONG_MIN = 3;
//     private static final int DIRECTION_30M_WEAK_MIN    = 2;

//     // =========================================================================
//     // NEW — 15M confirmation (does the 30M direction still hold one
//     // timeframe down?). Only 3 conditions: EMA9 vs EMA21, price vs EMA21,
//     // Supertrend. No slope, no 5-point scoring — this should confirm, not
//     // delay.
//     //   CONFIRM_15M_MIN = 2/3, and the opposite side must not outscore it.
//     // =========================================================================
//     private static final int CONFIRM_15M_MIN = 2;

//     // =========================================================================
//     // NEW — 3M entry trigger constants. 3M is the execution timeframe only —
//     // it never decides the overall trend, only whether the short-term move
//     // has already started in the already-confirmed 30M/15M/5M direction.
//     //   TRIGGER_3M_LOOKBACK = how many recent 3M bars (excluding the current
//     //     one) are searched for the high/low that must be broken.
//     // =========================================================================
//     private static final int TRIGGER_3M_LOOKBACK = 8;

//     // =========================================================================
//     // NEW — 5M pullback/recovery setup, RELAXED. The old version had 5
//     // simultaneous mandatory conditions (rejection wick, body>=40%, slope,
//     // directional candle, pullback) plus a 5-point confirmation score
//     // (volume, RSI, VWAP, Supertrend, EMA-align) requiring 3/5 — 10
//     // effective checks stacked together, which is what was causing entries
//     // to happen after a large chunk of the move was already gone.
//     //
//     // Now there is exactly ONE mandatory condition: has price pulled back
//     // near the 5M EMA9/EMA21 (i.e. not already extended)? Everything else
//     // (directional candle, RSI, volume) is a soft supporting signal, and
//     // only SETUP_5M_MIN_SUPPORT_SCORE of those 3 need to agree.
//     // =========================================================================
//     private static final double PULLBACK_MAX_ATR       = 1.5;
//     private static final double OVEREXTENSION_SKIP_ATR  = 2.0;
//     private static final double CANDLE_BODY_RATIO_MIN   = 0.40; // LEGACY — no longer used by the relaxed 5M setup
//     private static final int    SETUP_5M_MIN_SUPPORT_SCORE = 2; // out of 3 soft signals (directional candle, RSI, volume)
//     private static final double VOLUME_NOT_WEAK_FRACTION = 0.7; // volume just needs to be >=70% of average, not above it

//     // =========================================================================
//     // 5M supporting confirmation — RSI ranges widened slightly per the new
//     // spec (was 40-70 long / 30-60 short).
//     // =========================================================================
//     private static final int    ENTRY_CONFIRMATION_MIN_SCORE = 3; // LEGACY — unused by the relaxed 5M setup
//     private static final double RSI_LONG_MIN  = 45, RSI_LONG_MAX  = 70;
//     private static final double RSI_SHORT_MIN = 30, RSI_SHORT_MAX = 60;

//     // =========================================================================
//     // Pending breakout-entry signal. SHORTENED — the old 15-minute window
//     // matched a slower 5M-breakout arm/confirm cycle. Now that the 3M
//     // trigger itself supplies the breakout confirmation (see
//     // analyzeTrigger3M()), a stale setup should be discarded faster.
//     // =========================================================================
//     private static final long SIGNAL_MAX_VALID_MS = 9L * 60 * 1000L; // was 15 min — now ~3 x 3M candles
...
//     private static final double SL_MAX_PERCENT  = 10.0;  // SL can never be wider than this — "very very small" SL, tune here
...
//         boolean strongTrend;    // NEW: 30M scored a clean 4/4 at arm time -> eligible for RR_STRONG
//         long   createdAtMs;
//     }
...
//     // =========================================================================
//     // NEW — 30M direction scoring (4 conditions). This REPLACES the old
//     // 1H-mandatory + 6-point-30M cascade as the sole direction gate. 30M's
//     // only job is to establish the dominant intraday direction, not to wait
//     // for a perfect trend — so this is deliberately lighter than
//     // analyzeScored6() above (4 checks instead of 6, no structure/price-vs-
//     // EMA9 requirement).
//     // =========================================================================
//     private static class DirScore4 {
//         boolean valid;
//         int bullScore, bearScore; // each out of 4
//     }

//     private static DirScore4 scoreDirection30M(JSONArray candles) {
//         DirScore4 r = new DirScore4();
...
//         int bull = 0, bear = 0;
//         if (ema9 > ema21) bull++; else if (ema9 < ema21) bear++;
//         if (price > ema21) bull++; else if (price < ema21) bear++;
//         if (stGreen) bull++; else bear++;
//         if (slopeUp) bull++; else if (slopeDown) bear++;
...
//     // Decides the 30M direction from a DirScore4: TRUE=long, FALSE=short,
//     // null=skip (unclear or genuinely conflicting).
//     private static Boolean decide30MDirection(DirScore4 d) {
//         if (!d.valid) return null;
//         if (d.bullScore >= DIRECTION_30M_STRONG_MIN && d.bullScore > d.bearScore) return true;
//         if (d.bearScore >= DIRECTION_30M_STRONG_MIN && d.bearScore > d.bullScore) return false;
//         // Weak (2/4) is only accepted when the opposite side doesn't
//         // meaningfully disagree — a genuine 2-vs-2 split is a real conflict.
//         if (d.bullScore == DIRECTION_30M_WEAK_MIN && d.bearScore <= 1) return true;
//         if (d.bearScore == DIRECTION_30M_WEAK_MIN && d.bullScore <= 1) return false;
//         return null;
//     }

//     // =========================================================================
//     // NEW — 15M confirmation (3 conditions: EMA9 vs EMA21, price vs EMA21,
//     // Supertrend). Confirms the 30M direction one timeframe down; it should
//     // NOT be another heavy scoring gate that delays entry.
//     // =========================================================================
//     private static class DirScore3 {
//         boolean valid;
//         int bullScore, bearScore; // each out of 3
//     }

//     private static DirScore3 scoreConfirm15M(JSONArray candles) {
//         DirScore3 r = new DirScore3();
//         if (candles == null || candles.length() < EMA_MID + Math.max(ATR_PERIOD, ST_PERIOD)) {
//             r.valid = false;
//             return r;
//         }
...
//     // True if the 15M read agrees with the already-decided 30M direction
//     // (score >= CONFIRM_15M_MIN and the opposite side doesn't outscore it).
//     private static boolean confirms15M(DirScore3 d, boolean trendUp) {
//         if (!d.valid) return false;
//         int score = trendUp ? d.bullScore : d.bearScore;
//         int opposite = trendUp ? d.bearScore : d.bullScore;
//         return score >= CONFIRM_15M_MIN && opposite <= score;
//     }

//     // =========================================================================
//     // NEW — 3M entry trigger. 3M does NOT decide trend — it only checks
//     // whether the short-term move has already started in the direction the
//     // 30M/15M/5M timeframes have already agreed on. Trigger fires when the
//     // latest completed 3M candle breaks the recent 3M high/low (over
//     // TRIGGER_3M_LOOKBACK bars, excluding itself) AND at least one of the
//     // two supporting signals (EMA9/EMA21 momentum, directional candle)
//     // agrees — enough to filter pure noise without adding heavy conditions.
//     // =========================================================================
//     private static class Trigger3MResult {
//         boolean valid;
//         boolean triggered;
//         double confirmHigh, confirmLow; // latest completed 3M candle's high/low
//         String reason;
//     }

//     private static Trigger3MResult analyzeTrigger3M(JSONArray raw3m, boolean trendUp) {
//         Trigger3MResult t = new Trigger3MResult();
//         int minBars = EMA_MID + 5;
//         if (raw3m == null || raw3m.length() < minBars) {
//             t.valid = false;
//             return t;
//         }

//         double[] cl = extractCloses(raw3m);
//         double[] op = extractOpens(raw3m);
//         double[] hi = extractHighs(raw3m);
//         double[] lo = extractLows(raw3m);
//         int n = cl.length;

//         double ema9  = calcEMA(cl, EMA_FAST);
//         double ema21 = calcEMA(cl, EMA_MID);
//         boolean emaMomentumOk = trendUp ? ema9 > ema21 : ema9 < ema21;

//         double lastClose = cl[n - 1], lastOpen = op[n - 1];
//         boolean directionalCandle = trendUp ? (lastClose > lastOpen) : (lastClose < lastOpen);

//         int start = Math.max(0, n - 1 - TRIGGER_3M_LOOKBACK);
//         double recentHigh = Double.NEGATIVE_INFINITY, recentLow = Double.POSITIVE_INFINITY;
//         for (int i = start; i < n - 1; i++) { // exclude the current/last candle itself
//             recentHigh = Math.max(recentHigh, hi[i]);
//             recentLow  = Math.min(recentLow, lo[i]);
//         }

//         boolean breakout = trendUp ? lastClose > recentHigh : lastClose < recentLow;

//         t.triggered = breakout && (emaMomentumOk || directionalCandle);
//         t.confirmHigh = hi[n - 1];
//         t.confirmLow  = lo[n - 1];
//         t.valid = true;
//         t.reason = String.format("ema=%s directional=%s breakout=%s(level=%.6f) -> triggered=%s",
//                 emaMomentumOk, directionalCandle, breakout, trendUp ? recentHigh : recentLow, t.triggered);
//         return t;
//     }
...
//     // CHANGED — was 5 simultaneous mandatory conditions (pullback, directional
//     // candle, not-doji, rejection wick, slope) PLUS a 5-point confirmation
//     // score (volume, RSI, VWAP, 5M Supertrend, EMA-align) requiring 3/5 —
//     // effectively 10 checks stacked together, which was the main reason
//     // entries happened after a large chunk of the move had already gone.
//     //
//     // NEW: only ONE mandatory condition — has price pulled back near the 5M
//     // EMA9/EMA21 (i.e. not already extended)? Everything else (directional
//     // candle, RSI, volume) is now a soft supporting signal; only
//     // SETUP_5M_MIN_SUPPORT_SCORE of those 3 need to agree. The purpose of
//     // 5M is now just "has price pulled back and started recovering in the
//     // already-confirmed 30M/15M direction?" — the 3M timeframe supplies the
//     // actual, tighter entry trigger.
...
//         // ---- Soft supporting signals (2 of 3 needed) ----
...
//         // Relaxed: volume just needs to not be extremely weak (>=70% of
//         // average), not above-average like before.
//         boolean volumeOk = avgVol <= 0 || vol[n - 1] >= avgVol * VOLUME_NOT_WEAK_FRACTION;

//         int supportScore = (directionalCandle ? 1 : 0) + (rsiOk ? 1 : 0) + (volumeOk ? 1 : 0);

//         t.setupFound = pulledBack && supportScore >= SETUP_5M_MIN_SUPPORT_SCORE;
//         t.confirmHigh = entryHigh;
//         t.confirmLow  = entryLow;
//         t.valid = true;
//         t.reason = String.format(
//                 "pullback=%.2fATR(ok=%s) directional=%s rsi=%.1f(ok=%s) volOk=%s supportScore=%d/3",
//                 distanceAtr, pulledBack, directionalCandle, rsi, rsiOk, volumeOk, supportScore);
//         return t;
//     }
...
//                     }
//                     if (!cancelled) {
//                         // CHANGED — was a 1H re-check via analyzeScored6(). 1H is
//                         // no longer part of the entry decision at all; re-validate
//                         // against the NEW 30M direction instead.
//                         JSONArray raw30mCheck = aggregateCandles(raw5m, GROUP_30M_FROM_5M);
//                         DirScore4 dir30Check = scoreDirection30M(raw30mCheck);
//                         Boolean stillDir = decide30MDirection(dir30Check);
//                         if (stillDir == null || stillDir != pending.isLong) {
//                             System.out.println("  Signal cancelled (30M direction changed): " + pair);
//                             cancelled = true;
//                         }
//                     }
...
//                 // ---- No pending signal — look for a fresh one (NEW 30M -> 15M -> 5M -> 3M cascade) ----

//                 // STEP 1 — 30M: overall direction (loose, 4-condition)
//                 JSONArray raw30m = aggregateCandles(raw5m, GROUP_30M_FROM_5M);
//                 DirScore4 dir30 = scoreDirection30M(raw30m);
//                 Boolean trendUpObj = decide30MDirection(dir30);
//                 if (trendUpObj == null) {
//                     if (dir30.valid) {
//                         System.out.println("  [REJECT] " + pair + " — 30M direction unclear (bull="
//                                 + dir30.bullScore + "/4 bear=" + dir30.bearScore + "/4)");
//                     }
//                     continue;
//                 }
//                 boolean trendUp = trendUpObj;

//                 // STEP 2 — 15M: confirmation (loose, 3-condition)
//                 JSONArray raw15m = aggregateCandles(raw5m, GROUP_15M_FROM_5M);
//                 DirScore3 dir15 = scoreConfirm15M(raw15m);
//                 if (!confirms15M(dir15, trendUp)) {
//                     System.out.println("  [REJECT] " + pair + " — 15M conflict/weak (score="
//                             + (dir15.valid ? (trendUp ? dir15.bullScore : dir15.bearScore) : -1) + "/3, 30M was "
//                             + (trendUp ? "LONG" : "SHORT") + ")");
//                     continue;
//                 }

//                 // STEP 3 — 5M: relaxed pullback/recovery setup
//                 EntryResult entry5m = analyzeEntry5M(raw5m, trendUp);
//                 if (!entry5m.valid || !entry5m.setupFound) {
//                     System.out.println("  [REJECT] " + pair + " — 5M pullback/recovery not found. "
//                             + (entry5m.valid ? entry5m.reason : "insufficient 5M data"));
//                     continue;
//                 }

//                 // SIMPLE overextension check (ATR-based, off the 5M EMA area) —
//                 // do this before spending an extra API call on 3M data.
//                 if (entry5m.distanceAtr > OVEREXTENSION_SKIP_ATR) {
//                     System.out.println("  [REJECT] " + pair + " — price already overextended ("
//                             + String.format("%.2f", entry5m.distanceAtr) + " ATR from 5M EMA)");
//                     continue;
//                 }

//                 // STEP 4 — 3M: actual entry trigger. Native 3M candles aren't
//                 // offered by CoinDCX, so aggregate from 1M (see RES_1M/GROUP_3M_FROM_1M).
//                 JSONArray raw1m = dropLastIfForming(getCandlestickData(pair, RES_1M, BASE_1M_FETCH_COUNT));
//                 JSONArray raw3m = aggregateCandles(raw1m, GROUP_3M_FROM_1M);
//                 Trigger3MResult trig3m = analyzeTrigger3M(raw3m, trendUp);
//                 if (!trig3m.valid || !trig3m.triggered) {
//                     System.out.println("  [REJECT] " + pair + " — 3M trigger not confirmed. "
//                             + (trig3m.valid ? trig3m.reason : "insufficient 3M data"));
//                     continue;
//                 }

//                 // Capture the pullback swing NOW (5M) — unchanged SL anchor logic,
//                 // still the raw structural swing + ATR buffer used later by
//                 // slTpFromLevel()/structuralSlTooWide(). Not affected by the
//                 // entry-cascade rewrite.
