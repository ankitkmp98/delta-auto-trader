...
public class CoinDCXFuturesTrader8C_BUY_SELL_NEW_LOGIC_THREE {
    ...
    // === STRATEGY UPDATE: SL/TP now purely 15M-Supertrend + ATR based ===
    // stBands15 = {lowerBand, upperBand} from calcSupertrendBands() on 15M data,
    // atr15 = calcATR(14) on 15M data. Used both by the live scan (real entry
    // or a pre-fill current-price estimate) and by the safety sweep.
    //
    //   Long:  SL = 15M-Supertrend lower band - (0.3 x ATR15)
    //          TP = Entry + (Entry - SL) x 2   [RR_TARGET = 2.0]
    //   Short: SL = 15M-Supertrend upper band + (0.3 x ATR15)
    //          TP = Entry - (SL - Entry) x 2
    private static double[] computeSlTp(boolean isLong, double entryPrice, double[] stBands15,
                                         double atr15, double tickSize) {
        double sl, tp;
        if (isLong) {
            sl = stBands15[0] - SL_ATR_BUFFER * atr15;
            double risk = entryPrice - sl;
            tp = entryPrice + RR_TARGET * risk;
        } else {
            sl = stBands15[1] + SL_ATR_BUFFER * atr15;
    ...
    public static void main(String[] args) {
        ...
        for (String pair : COINS_TO_TRADE) {
            try {
                ...
                // === STRATEGY UPDATE: 1H trend filter (MANDATORY) ===
                // Long:  EMA9 > EMA21 AND price > EMA9 AND price > EMA21
                // Short: EMA9 < EMA21 AND price < EMA9 AND price < EMA21
                ...
                boolean longOk1h  = tf1h.ema9 > tf1h.ema21 && tf1h.price > tf1h.ema9 && tf1h.price > tf1h.ema21;
                boolean shortOk1h = tf1h.ema9 < tf1h.ema21 && tf1h.price < tf1h.ema9 && tf1h.price < tf1h.ema21;
                System.out.printf("  [1H] EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        tf1h.ema9, tf1h.ema21, tf1h.price,
                ...
                // === STRATEGY UPDATE: 30M trend filter (MANDATORY) — must agree with 1H ===
                TFResult tf30m = analyzeTF(raw30m);
                if (!tf30m.valid) {
                    System.out.println("  [30M] insufficient data — skip"); continue;
                }
                boolean longOk30  = tf30m.ema9 > tf30m.ema21 && tf30m.price > tf30m.ema9 && tf30m.price > tf30m.ema21;
                boolean shortOk30 = tf30m.ema9 < tf30m.ema21 && tf30m.price < tf30m.ema9 && tf30m.price < tf30m.ema21;
                boolean tf30mAligned = trendUp ? longOk30 : shortOk30;
                System.out.printf("  [30M] EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        tf30m.ema9, tf30m.ema21, tf30m.price, tf30mAligned ? "ALIGNED" : "NOT ALIGNED");
                if (!tf30mAligned) {
                ...
                // === STRATEGY UPDATE: 15M entry confirmation (MANDATORY) ===
                ...
                boolean longOk15Trend  = ema9_15 > ema21_15 && entryClose > ema9_15 && entryClose > ema21_15 && stBull15;
                boolean shortOk15Trend = ema9_15 < ema21_15 && entryClose < ema9_15 && entryClose < ema21_15 && !stBull15;
                boolean tf15TrendOk = trendUp ? longOk15Trend : shortOk15Trend;
                System.out.printf("  [15M] ST=%s EMA9=%.6f EMA21=%.6f Price=%.6f → %s%n",
                        stBull15 ? "GREEN" : "RED", ema9_15, ema21_15, entryClose,
                        tf15TrendOk ? "ALIGNED" : "NOT ALIGNED");
                if (!tf15TrendOk) {
                    System.out.println("  15M FAIL — trend/Supertrend filter not satisfied — skip"); continue;
                }
                ...
                System.out.printf("  [15M-Pullback] distEMA9=%.6f distST=%.6f maxAllowed=%.6f → %s%n",
                ...
                // === STRATEGY UPDATE: ADX(14) > 25 filter ===
                double adx15 = calcADX(hi15, lo15, cl15, ADX_PERIOD);
                boolean adxOk = adx15 > ADX_THRESHOLD;
                System.out.printf("  [15M-ADX] ADX(%d)=%.2f (need > %.1f) → %s%n",
                        ADX_PERIOD, adx15, ADX_THRESHOLD, adxOk ? "PASS" : "FAIL");
                if (!adxOk) {
                    System.out.println("  15M FAIL — ADX below threshold, trend not strong enough — skip"); continue;
                }

                // === STRATEGY UPDATE: Volume > 20-period Volume EMA filter ===
                double[] vol15   = extractVolumes(raw15m);
                double volEma15  = calcEMA(vol15, VOLUME_EMA_PERIOD);
                double currentVol = vol15[n15 - 1];
                boolean volOk = currentVol > volEma15;
                System.out.printf("  [15M-Volume] current=%.2f | EMA%d=%.2f → %s%n",
                        currentVol, VOLUME_EMA_PERIOD, volEma15, volOk ? "PASS" : "FAIL");
                if (!volOk) {
                    System.out.println("  15M FAIL — volume below 20-period volume EMA — skip"); continue;
                }
                System.out.println("  15M OK — trend + pullback + rejection + ADX + volume all confirmed");
                ...
                // the same 15M-Supertrend + 0.3xATR15 buffer that the real SL will use.
                // calcQuantityByRisk() itself is unchanged from the previous version.
                double[] preSlTp = computeSlTp(trendUp, currentPrice, stBands15, atr15, tickSize);
                ...
                double[] slTp = computeSlTp(trendUp, entry, stBands15, atr15, tickSize);
    ...
    private static void ensureTpSlForOpenPositions() {
        try {
            ...
            for (String pair : stillOpen) {
                ...
                // === STRATEGY UPDATE: sweep now derives SL/TP from 15M Supertrend + ATR,
                // same basis as the live scan (was 2H-aggregated data before).
                JSONArray raw15mSweep = getCandlestickData(pair, "15", CANDLE_15M);
                if (raw15mSweep == null || raw15mSweep.length() < EMA_MID + 5) {
                    System.out.println("  [SWEEP] insufficient 15M data for " + pair + " — will retry next run");
                    continue;
                }
                double[] hi15Sweep = extractHighs(raw15mSweep);
                double[] lo15Sweep = extractLows(raw15mSweep);
                double[] cl15Sweep = extractCloses(raw15mSweep);
                double atr15Sweep = calcATR(hi15Sweep, lo15Sweep, cl15Sweep, ATR_PERIOD);
                double[] stBands15Sweep = calcSupertrendBands(hi15Sweep, lo15Sweep, cl15Sweep, ST_PERIOD, ST_MULTIPLIER);

                double posQty = pos.optDouble("active_pos", 0);
                boolean isLong = posQty >= 0;

                double tick = getTickSize(pair);
                double[] slTp = computeSlTp(isLong, avgPrice, stBands15Sweep, atr15Sweep, tick);
